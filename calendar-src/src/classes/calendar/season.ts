...
export default class Season extends ConfigurationItemBase {
     ...
     */
    toConfig(): FalloutMaWCalendar.SeasonData {
        return {
            id: this.id,
            name: this.name,
     ...
     */
    toTemplate(): FalloutMaWCalendar.HandlebarTemplateData.Season {
        const startDateSelectorId = `fallout_maw_calendar_season_start_date_${this.id}`;
        const sunriseSelectorId = `fallout_maw_calendar_season_sunrise_time_${this.id}`;
     ...
     */
    loadFromSettings(config: FalloutMaWCalendar.SeasonData) {
