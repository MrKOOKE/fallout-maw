import { GameWorldTimeIntegrations, LeapYearRules, FalloutMaWCalendarHooks, TimeKeeperStatus } from "../../constants";
...
export default class Calendar extends ConfigurationItemBase {
     ...
     */
    public noteCategories: FalloutMaWCalendar.NoteCategory[] = [];
    /**
     ...
     */
    constructor(id: string, name: string, configuration: FalloutMaWCalendar.CalendarData = { id: "" }) {
        super(name);
     ...
     */
    toTemplate(): FalloutMaWCalendar.HandlebarTemplateData.Calendar {
        ...
        return {
            ...super.toTemplate(),
            calendarDisplayId: `fallout_maw_calendar_${this.id}_calendar`,
            clockDisplayId: `fallout_maw_calendar_${this.id}_clock`,
     ...
     */
    toConfig(): FalloutMaWCalendar.CalendarData {
        return <FalloutMaWCalendar.CalendarData>{
            id: this.id,
            name: this.name,
     ...
     * @param config
     */
    loadFromSettings(config: FalloutMaWCalendar.CalendarData) {
        if (config.id) {
            this.id = config.id;
        ...
        const configMonths: FalloutMaWCalendar.MonthData[] | undefined = config.months || config.monthSettings;
        if (Array.isArray(configMonths)) {
            ...
            if (this.months.length === 0) {
                this.months.push(new Month(GameSettings.Localize("FALLOUTMAW.Calendar.Defaults.Month"), 1, 0, 30));
            }
        }
        const configWeekdays: FalloutMaWCalendar.WeekdayData[] | undefined = config.weekdays || config.weekdaySettings;
        ...
        const configSeasons: FalloutMaWCalendar.SeasonData[] | undefined = config.seasons || config.seasonSettings;
        ...
        const configMoons: FalloutMaWCalendar.MoonData[] | undefined = config.moons || config.moonSettings;
     ...
     * @private
     */
    getCurrentDate(): FalloutMaWCalendar.CurrentDateData {
     ...
     */
    getDateTime(): FalloutMaWCalendar.DateTime {
        const dt: FalloutMaWCalendar.DateTime = {
            year: 0,
            month: 0,
            day: 0,
            hour: 0,
    ...
    getMonthAndDayIndex(setting: string = "current") {
        const verifiedSetting = setting.toLowerCase() as "visible" | "current" | "selected";
        const result: Partial<FalloutMaWCalendar.Date> = {
            month: 0,
            day: 0
        };
     ...
     */
    daysIntoWeeks(monthIndex: number, year: number, weekLength: number): (boolean | FalloutMaWCalendar.HandlebarTemplateData.Day)[][] {
        const weeks = [];
     ...
     */
    secondsToDate(seconds: number): FalloutMaWCalendar.DateTime {
     ...
     * @param seconds
     */
    secondsToInterval(seconds: number): FalloutMaWCalendar.DateTimeParts {
        let sec = seconds,
            min = 0,
            hour = 0,
            month: number;
    ...
    public changeDateTime(interval: FalloutMaWCalendar.DateTimeParts, options: FalloutMaWCalendar.DateChangeOptions = {}) {
        ...
        if (canUser((<Game>game).user, SC.globalConfiguration.permissions.changeDateTime)) {
            ...
            if (change) {
                if (CalManager.isActiveCalendar(this.id)) {
                    ...
                    Hook.emit(FalloutMaWCalendarHooks.DateTimeChange, this, changeInSeconds);
                ...
                if (
                    options.save &&
                    (this.generalSettings.gameWorldTimeIntegration === GameWorldTimeIntegrations.None || CalManager.syncWithAllCalendars)
                ) {
            ...
            GameSettings.UiNotification(GameSettings.Localize("FALLOUTMAW.Calendar.Warn.Macros.GMUpdate"), "warn");
        }
        return false;
    }

    public setDateTime(date: FalloutMaWCalendar.DateTimeParts, options: FalloutMaWCalendar.DateChangeOptions = {}) {
        options = deepMerge(
            {},
        ...
        if (canUser((<Game>game).user, SC.globalConfiguration.permissions.changeDateTime) || options.bypassPermissionCheck) {
            ...
            const processedDate: FalloutMaWCalendar.DateTime = {
                year: date.year || 0,
                month: date.month || 0,
                day: date.day || 0,
                hour: date.hour || 0,
    ...
    async syncTime(force: boolean = false) {
        ...
        if (
            ...
            if (totalSeconds !== (<Game>game).time.worldTime || force) {
                ...
                try {
                    const applied = await this.time.setWorldTime(totalSeconds);
                    if (!applied) {
                        this.restoreFromWorldTime();
                    ...
                    if (this.timeKeeper.getStatus() !== TimeKeeperStatus.Started) {
                        await CalManager.saveCalendars();
                    }
                } catch (error) {
                    this.restoreFromWorldTime();
     ...
     */
    setFromTime(newTime: number, changeAmount: number, options: Record<string, any> = {}) {
        ...
        if (changeAmount !== 0 && !this.timeChangeTriggered) {
            const integration = this.generalSettings.gameWorldTimeIntegration;
            const source = String(options?.falloutMawWorldTimeSource ?? "");
            const followsWorldTime = integration === GameWorldTimeIntegrations.Mixed || integration === GameWorldTimeIntegrations.ThirdParty;
            const acceptedSelfUpdate = integration === GameWorldTimeIntegrations.Self && (source === "calendar" || this.combatChangeTriggered);
            if (followsWorldTime || acceptedSelfUpdate) {
                ...
                Renderer.Clock.UpdateListener(`fallout_maw_calendar_${this.id}_clock`, this.timeKeeper.getStatus());
            }
        }
        this.timeChangeTriggered = false;
    ...
    processOwnCombatRoundTime(combat: Combat) {
        ...
        if (roundSeconds !== 0 && roundsPassed !== 0) {
            ...
            this.changeDateTime(
                { seconds: roundSeconds * roundsPassed },
                { updateApp: false, sync: true, save: GameSettings.IsGm() && SC.primary }
            );
        }
    }

    /** Restore a speculative calendar change after the canonical world-time write failed. */
    private restoreFromWorldTime() {
        this.timeChangeTriggered = false;
        this.combatChangeTriggered = false;
        const parsedDate = this.secondsToDate((<Game>game).time.worldTime);
        this.setDateTime(parsedDate, {
            updateApp: true,
            sync: false,
            save: false,
     ...
     */
    updateTime(parsedDate: FalloutMaWCalendar.DateTime) {
