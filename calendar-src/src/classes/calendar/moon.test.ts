...
describe("Moon Tests", () => {
    ...
    test("Properties", () => {
        expect(Object.keys(m).length).toBe(13); //Make sure no new properties have been added
        expect(m.name).toBe("Moon");
        expect(m.cycleLength).toBe(29.53059);
        expect(m.cycleDayAdjust).toBe(0.5);
        expect(m.color).toBe("#ffffff");
        expect(m.affectsLighting).toBe(true);
        expect(m.maxLightContribution).toBe(0.5);
        expect(m.phases.length).toBe(8);
        expect(m.phases.map((phase) => phase.illumination)).toStrictEqual([0, 0.25, 0.5, 0.75, 1, 0.75, 0.5, 0.25]);
    ...
    test("Update Phase Length", () => {
        ...
        m.phases.push({ name: "p2", icon: Icons.NewMoon, length: 0, singleDay: false, illumination: 0 });
        m.updatePhaseLength();
        expect(m.phases[0].length).toBe(1);
