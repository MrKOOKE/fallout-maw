...
import { GameWorldTimeIntegrations, PredefinedCalendars, SettingNames, FalloutMaWCalendarHooks, SocketTypes, TimeKeeperStatus } from "../../constants";
...
export default class CalendarManager {
    ...
    public async saveCalendars() {
        const calendarConfigurations: FalloutMaWCalendar.CalendarData[] = [];
    ...
    /** Persist standalone clock timestamps without rewriting every calendar configuration. */
    public async saveClockTime(calendar: Calendar) {
        const state = { ...(<Record<string, number>>GameSettings.GetObjectSettings(SettingNames.ClockTime)) };
        state[calendar.id] = calendar.toSeconds();
        await GameSettings.SaveObjectSetting(SettingNames.ClockTime, state);
    }

    /** Apply compact standalone clock timestamps received from the world setting. */
    public loadClockTimes() {
        const state = <Record<string, number>>GameSettings.GetObjectSettings(SettingNames.ClockTime);
        if (!state || typeof state !== "object") return;
        for (const [calendarId, value] of Object.entries(state)) {
            const calendar = this.getCalendar(calendarId);
            const timestamp = Number(value);
            if (!calendar || !Number.isFinite(timestamp)) continue;
            if (calendar.generalSettings.gameWorldTimeIntegration !== GameWorldTimeIntegrations.None) continue;
            if (calendar.toSeconds() === timestamp) continue;
            calendar.setDateTime(calendar.secondsToDate(timestamp), {
                updateApp: this.isActiveCalendar(calendarId) && calendar.timeKeeper.getStatus() !== TimeKeeperStatus.Started,
                sync: false,
                save: false,
    ...
    public loadCalendars(): number {
        ...
        const calendars = <FalloutMaWCalendar.CalendarData[]>GameSettings.GetObjectSettings(SettingNames.CalendarConfiguration);
            ...
            for (let i = 0; i < calendars.length; i++) {
                if (calendars[i].id) {
                    ...
                    if (existingCalendar) {
                        ...
                        existingCalendar = this.addCalendar(
                            calendars[i].id,
                            calendars[i].name ||
                                GameSettings.Localize("FALLOUTMAW.Calendar.Defaults.NumberedCalendar").replace("{NUMBER}", i.toString()),
                            calendars[i]
                        );
                    }
     ...
     */
    public addCalendar(id: string, name: string, configuration: FalloutMaWCalendar.CalendarData): Calendar {
    ...
    public setActiveCalendar(id: string) {
        ...
        if (newActive) {
            for (const [, value] of Object.entries(this.calendars)) {
                if (value.id === newActive.id) continue;
                value.timeKeeper.pause();
                value.timeKeeper.relinquishAuthority();
            }
            ...
            const clockStatus = newActive.timeKeeper.getStatus();
            GameSockets.emit(<FalloutMaWCalendar.FalloutMaWCalendarSocket.Data>{
                type: SocketTypes.clock,
                data: newActive.generalSettings.gameWorldTimeIntegration === GameWorldTimeIntegrations.None
                    ? { status: clockStatus, timestamp: newActive.toSeconds() }
                    : clockStatus
            }).catch(Logger.error);
            MainApplication.clockClass = clockStatus;
            GameSettings.SaveStringSetting(SettingNames.ClockState, clockStatus).catch(Logger.error);
            ...
            Hook.emit(FalloutMaWCalendarHooks.DateTimeChange, newActive);
        }
    }
     ...
     * @param interval
     */
    public syncWithCalendars(interval: FalloutMaWCalendar.DateTimeParts) {
