...
import { LocalizeCalendarDisplayTerm } from "../utilities/display-localization";
...
export default class MainApp extends FormApplication {
     ...
     * @type {string}
     */
    public static appWindowId: string = "fallout-maw-calendar-application";
    /**
    ...
    uiElementStates: Record<string, boolean | string> = {
        "fallout-maw-calendar-calendar-list": false,
        "fallout-maw-calendar-note-list": false,
        "fallout-maw-calendar-note-search": false,
        "fallout-maw-calendar-time-options": false,
        compactView: false,
        dateTimeUnitOpen: false,
        dateTimeUnit: DateTimeUnits.Day,
        dateTimeUnitText: "FALLOUTMAW.Calendar.Day",
        searchOptionsOpen: false,
        calendarListOpen: false,
        primaryCheckRunning: false,
        cvReverseTime: false,
        cvLargerSteps: false
    };
    ...
    search = {
        ...
        results: <NoteStub[]>[],
        options: {
            fields: <FalloutMaWCalendar.Search.OptionsFields>{
                date: true,
                title: true,
                details: true,
                author: true,
    ...
    addonButtons: FalloutMaWCalendar.AddonButton[] = [];
    /**
    ...
    initialize() {
        ...
        this.uiElementStates["fallout-maw-calendar-note-list"] = GameSettings.GetBooleanSettings(SettingNames.AlwaysShowNoteList);
    ...
    static get defaultOptions() {
        ...
        options.template = "systems/fallout-maw/calendar/templates/main.html";
        options.title = "FALLOUTMAW.Calendar.Title";
        options.classes = ["fallout-maw-calendar"];
    ...
    async getData(): Promise<GetDataReturnType<FormApplication.FormApplicationData>> {
        ...
        const data = {
            ...
            timeOptions,
            timeOptionLabels: {
                title: GameSettings.Localize("FALLOUTMAW.Calendar.TimeOptions.Title"),
                restMode: GameSettings.Localize("FALLOUTMAW.Calendar.TimeOptions.RestMode"),
                ignoreTimeMechanics: GameSettings.Localize("FALLOUTMAW.Calendar.TimeOptions.IgnoreTimeMechanics"),
                needsPlayersOnly: GameSettings.Localize("FALLOUTMAW.Calendar.TimeOptions.NeedsPlayersOnly")
            },
            dateTimeFullDisplay: {
        ...
        if (this.activeCalendar.id === this.visibleCalendar.id) {
            ...
            data.message = GameSettings.Localize("FALLOUTMAW.Calendar.ViewingDifferentCalendar");
        }
        if (this.uiElementStates.compactView) {
            const season = this.visibleCalendar.getCurrentSeason();
            const seasonName = LocalizeCalendarDisplayTerm(season.name);
            data.compactViewDisplay.currentSeasonName = seasonName;
            ...
            if (data.compactViewDisplay.currentSeasonIcon === "") {
                data.compactViewDisplay.currentSeasonIcon = `<span style="color:${season.color};">${seasonName.slice(0, 2)}</span>`;
            }
            ...
            if (this.visibleCalendar.moons.length) {
                for (let i = 0; i < this.visibleCalendar.moons.length; i++) {
                    ...
                    data.compactViewDisplay.selectedDayMoons.push({
                        name: LocalizeCalendarDisplayTerm(this.visibleCalendar.moons[i].name),
                        color: this.visibleCalendar.moons[i].color,
                        phase: { ...phase, name: LocalizeCalendarDisplayTerm(phase.name) },
            ...
            for (let i = 0; i < this.addonButtons.length; i++) {
                data.mainViewDisplay.addonButtons += `<button class="fallout-maw-calendar-control fallout-maw-calendar-grey fallout-maw-calendar-addon-button ${this.addonButtons[i].customClass}" data-fallout-maw-calendar-abi="${i}" data-tooltip="${this.addonButtons[i].title}"><span class="fa ${this.addonButtons[i].iconClass}"></span></button>`;
                if (this.addonButtons[i].showSidePanel) {
                    if (this.uiElementStates[`fallout-maw-calendar-addon-button-side-drawer-${i}`] === undefined) {
                        this.uiElementStates[`fallout-maw-calendar-addon-button-side-drawer-${i}`] = false;
                    }
                    data.mainViewDisplay.addonButtonSidePanels += `<div class="fallout-maw-calendar-side-drawer fallout-maw-calendar-addon-button-side-drawer-${i} fallout-maw-calendar-addon-button-side-drawer ${
                        data.sideDrawerDirection
                    } ${this.uiElementStates[`fallout-maw-calendar-addon-button-side-drawer-${i}`] ? "fallout-maw-calendar-open" : "fallout-maw-calendar-closed"}" data-fallout-maw-calendar-abi="${i}"></div>`;
                }
            }
            ...
            data.mainViewDisplay.calendarList = CalManager.getAllCalendars().map((c) => {
                ...
                return {
                    id: c.id,
                    name: LocalizeCalendarDisplayTerm(c.name),
                    date: FormatDateTime(
    ...
    render(force?: boolean, options?: Application.RenderOptions<FormApplication.Options>): this {
        ...
        if (canUser((<Game>game).user, SC.globalConfiguration.permissions.viewCalendar)) {
            ...
            const persistentClass = SC.clientSettings.persistentOpen ? "fallout-maw-calendar-persistent" : "";

            let scaleClass = "";
            if (this.uiElementStates.compactView) {
                scaleClass = `fallout-maw-calendar-scale-${SC.clientSettings.compactViewScale}`;
            }

            mergedOptions.classes = ["fallout-maw-calendar", GetThemeName(), persistentClass, scaleClass];
    ...
    private getTimeOptions(): { restMode: boolean; ignoreTimeMechanics: boolean; needsPlayersOnly: boolean } {
        const defaults = { restMode: false, ignoreTimeMechanics: false, needsPlayersOnly: true };
        try {
            const getOptions = (game as any).falloutMaW?.calendar?.getTimeOptions;
            if (typeof getOptions !== "function") return defaults;
            const options = getOptions();
            return {
                restMode: Boolean(options?.restMode),
                ignoreTimeMechanics: Boolean(options?.ignoreTimeMechanics),
                needsPlayersOnly: options?.needsPlayersOnly === undefined ? defaults.needsPlayersOnly : Boolean(options.needsPlayersOnly)
            };
        } catch (error: any) {
    ...
    private async timeOptionChange(event: Event) {
        if (!GameSettings.IsGm()) return;
        const input = event.currentTarget as HTMLInputElement;
        const option = input.dataset.timeOption;
        if (option !== "restMode" && option !== "ignoreTimeMechanics" && option !== "needsPlayersOnly") return;

        const previous = this.getTimeOptions();
        input.disabled = true;
        try {
            const setOptions = (game as any).falloutMaW?.calendar?.setTimeOptions;
            if (typeof setOptions !== "function") throw new Error("Fallout MaW calendar time options bridge is unavailable");
            const updated = await setOptions({ [option]: input.checked });
            const values = updated && typeof updated === "object" ? updated : this.getTimeOptions();
            document
                .querySelectorAll<HTMLInputElement>(`#${MainApp.appWindowId} .fallout-maw-calendar-time-options input[data-time-option]`)
                .forEach((timeOptionInput) => {
                    const key = timeOptionInput.dataset.timeOption;
                    if (key && Object.prototype.hasOwnProperty.call(values, key)) timeOptionInput.checked = Boolean(values[key]);
                });
        } catch (error: any) {
            input.checked = previous[option];
            Logger.error(error instanceof Error ? error : String(error));
        } finally {
    ...
    public static setWidthHeight(ma: MainApp) {
        ...
        if (main) {
            ...
            const wrapper = <HTMLElement>main.querySelector(".fallout-maw-calendar-main-wrapper");
            if (wrapper) {
                if (ma.uiElementStates.compactView) {
                    wrapper.querySelectorAll(".fallout-maw-calendar-section").forEach((e) => {
                        height += (<HTMLElement>e).offsetHeight;
                    });
                    const minCalendarWidth = 225;
                    const currentDate = <HTMLElement>wrapper.querySelector(".fallout-maw-calendar-date .fallout-maw-calendar-date-text");
                    const controls = wrapper.querySelectorAll(".fallout-maw-calendar-unit-controls .fallout-maw-calendar-control-group");
                    let curDateWidth = 0,
                        controlWidth = 0;
                    if (currentDate) {
                    ...
                    wrapper.querySelectorAll(".fallout-maw-calendar-section").forEach((s) => {
                        height += (<HTMLElement>s).offsetHeight;
                    });
                    const minCalendarWidth = 200;
                    const currentDate = <HTMLElement>wrapper.querySelector(".fallout-maw-calendar-calendar .fallout-maw-calendar-calendar-header .fallout-maw-calendar-current-date");
                    const week = <HTMLElement>wrapper.querySelector(".fallout-maw-calendar-calendar .fallout-maw-calendar-days .fallout-maw-calendar-week");
                    const clock = <HTMLElement>wrapper.querySelector(".fallout-maw-calendar-clock-display .fallout-maw-calendar-clock");
                    const yearView = <HTMLElement>wrapper.querySelector(".fallout-maw-calendar-year-view");
                    let weekWidth = 0,
                        clockWidth = 0,
                        yearViewWidth = 0;
            ...
            if (ma.uiElementStates.compactView && GameSettings.GetBooleanSettings(SettingNames.RememberCompactPosition)) {
                const pos = <FalloutMaWCalendar.AppPosition>GameSettings.GetObjectSettings(SettingNames.AppCompactPosition);
                ...
                const pos = <FalloutMaWCalendar.AppPosition>GameSettings.GetObjectSettings(SettingNames.AppPosition);
                if (pos.top !== undefined && pos.top >= 0) {
                    options.top = pos.top;
                }
                if (pos.left !== undefined && pos.left >= 0) {
    ...
    ensureCurrentDateIsVisible() {
        const calendar = document.querySelector(`#${this.id} .fallout-maw-calendar-calendar`);
        ...
        if (calendar && calendarHeight && calendarHeight >= 500) {
            const currentDay = calendar.querySelector(".fallout-maw-calendar-day.fallout-maw-calendar-current");
            const selectedDay = calendar.querySelector(".fallout-maw-calendar-day.fallout-maw-calendar-selected");
    ...
    public appDragEnd(e: Event) {
        ...
        if (app) {
            const appPos: FalloutMaWCalendar.AppPosition = {};
            if (app.classList.contains("fallout-maw-calendar-compact-view")) {
    ...
    public activateListeners() {
        ...
        if (appWindow) {
            ...
            if (this.uiElementStates.compactView) {
                appWindow.classList.add("fallout-maw-calendar-compact-view");
            } else {
                appWindow.classList.remove("fallout-maw-calendar-compact-view");
                // Activate the full calendar display listeners
                Renderer.CalendarFull.ActivateListeners(
                    `fallout_maw_calendar_${this.visibleCalendar.id}_calendar`,
                    this.changeMonth.bind(this),
                    this.dayClick.bind(this)
                );
            }
            // Activate the clock listeners
            Renderer.Clock.ActivateListeners(`fallout_maw_calendar_${this.visibleCalendar.id}_clock`);

            //-----------------------
            // Calendar Action List
            //-----------------------
            // Calendar List Click
            appWindow.querySelector(".fallout-maw-calendar-actions-list .fallout-maw-calendar-calendars")?.addEventListener("click", this.toggleDrawer.bind(this, "fallout-maw-calendar-calendar-list"));
            //Configuration Button Click
            appWindow.querySelector(".fallout-maw-calendar-actions-list .fallout-maw-calendar-configure-button")?.addEventListener("click", this.configurationClick.bind(this));
            //Search button click
            appWindow.querySelector(".fallout-maw-calendar-actions-list .fallout-maw-calendar-search")?.addEventListener("click", this.toggleDrawer.bind(this, "fallout-maw-calendar-note-search"));
            // System time options drawer
            appWindow.querySelector(".fallout-maw-calendar-actions-list .fallout-maw-calendar-time-options-button")?.addEventListener("click", this.toggleDrawer.bind(this, "fallout-maw-calendar-time-options"));
            appWindow.querySelectorAll<HTMLInputElement>(".fallout-maw-calendar-time-options input[data-time-option]").forEach((input) => {
                input.addEventListener("change", this.timeOptionChange.bind(this));
            });
            // Add new note click
            appWindow.querySelector(".fallout-maw-calendar-add-note")?.addEventListener("click", this.addNote.bind(this));
            // Note Drawer Toggle
            appWindow.querySelector(".fallout-maw-calendar-notes")?.addEventListener("click", this.toggleDrawer.bind(this, "fallout-maw-calendar-note-list"));
            appWindow.querySelector(".fallout-maw-calendar-reminder-notes")?.addEventListener("click", this.toggleDrawer.bind(this, "fallout-maw-calendar-note-list"));
            // Today button click
            appWindow.querySelector(".fallout-maw-calendar-actions-list .fallout-maw-calendar-today")?.addEventListener("click", this.todayClick.bind(this));
            // Set Current Date
            appWindow.querySelector(".fallout-maw-calendar-actions-list .fallout-maw-calendar-btn-apply")?.addEventListener("click", this.dateControlApply.bind(this));
            // Real Time Clock
            appWindow.querySelector(".fallout-maw-calendar-time-start")?.addEventListener("click", this.startTime.bind(this));
            appWindow.querySelector(".fallout-maw-calendar-time-stop")?.addEventListener("click", this.stopTime.bind(this));

            //-----------------------
            // Calendar Drawer
            //-----------------------
            //Calendar Active Click
            const calendarActivate = appWindow.querySelectorAll(".fallout-maw-calendar-calendar-list .fallout-maw-calendar-calendar-display .fallout-maw-calendar-calendar-actions .fallout-maw-calendar-save");
            ...
            //Calendar View Click
            appWindow.querySelectorAll(".fallout-maw-calendar-calendar-list .fallout-maw-calendar-calendar-display").forEach((e) => {
            ...
            //-----------------------
            // Note Click/Drag
            appWindow.querySelectorAll(".fallout-maw-calendar-note-list .fallout-maw-calendar-note").forEach((n) => {
            ...
            appWindow.querySelectorAll(".fallout-maw-calendar-note-search .fallout-maw-calendar-note-list .fallout-maw-calendar-note").forEach((n) => {
                n.addEventListener("click", this.viewNote.bind(this));
            });
            //Search Click
            appWindow.querySelector(".fallout-maw-calendar-note-search .fallout-maw-calendar-search-box .fa-search")?.addEventListener("click", this.searchClick.bind(this));
            //Search Clear Click
            appWindow.querySelector(".fallout-maw-calendar-note-search .fallout-maw-calendar-search-box .fa-times")?.addEventListener("click", this.searchClearClick.bind(this));
            //Search Input Key Up
            appWindow.querySelector(".fallout-maw-calendar-note-search .fallout-maw-calendar-search-box input")?.addEventListener("keyup", this.searchBoxChange.bind(this));
            //Search Options Header Click
            appWindow
                .querySelector(".fallout-maw-calendar-note-search .fallout-maw-calendar-search-options-header")
                ?.addEventListener("click", this.searchOptionsToggle.bind(this, false));
            //Search Options Fields Change
            appWindow.querySelectorAll(".fallout-maw-calendar-note-search .fallout-maw-calendar-search-fields input").forEach((n) => {
                n.addEventListener("change", this.searchOptionsFieldsChange.bind(this));
            });
            //Context Item Click
            appWindow.querySelectorAll(".fallout-maw-calendar-section > .fallout-maw-calendar-context-menu .fallout-maw-calendar-context-list-action").forEach((e) => {
            ...
            // Date/Time Controls
            //-----------------------
            appWindow.querySelectorAll(".fallout-maw-calendar-unit-controls .fallout-maw-calendar-selector").forEach((s) => {
                s.addEventListener("click", this.toggleUnitSelector.bind(this, false));
            });
            appWindow.querySelectorAll(".fallout-maw-calendar-unit-controls .fallout-maw-calendar-unit-list li").forEach((c) => {
                c.addEventListener("click", this.changeUnit.bind(this));
            });
            appWindow.querySelectorAll(".fallout-maw-calendar-controls .fallout-maw-calendar-control").forEach((c) => {
            ...
            // Addon Buttons
            //-----------------------
            appWindow.querySelectorAll(".fallout-maw-calendar-addon-button").forEach((s) => {
    ...
    async addonButtonClick(event: Event) {
        const attr = (<HTMLElement>event.target)?.closest("button")?.attributes.getNamedItem("data-fallout-maw-calendar-abi");
        if (attr && attr.value) {
            const index = parseInt(attr.value);
            if (!isNaN(index) && index >= 0 && index < this.addonButtons.length) {
                const ab = this.addonButtons[index];
                if (ab.showSidePanel) {
                    this.toggleDrawer(`fallout-maw-calendar-addon-button-side-drawer-${index}`);
                } else {
                    try {
    ...
    addonButtonSidePanelContentRender(main: HTMLElement) {
        main.querySelectorAll(".fallout-maw-calendar-addon-button-side-drawer").forEach(async (sd) => {
            const attr = sd.attributes.getNamedItem("data-fallout-maw-calendar-abi");
            if (attr && attr.value) {
                const index = parseInt(attr.value);
                if (!isNaN(index) && index >= 0 && index < this.addonButtons.length) {
                    const ab = this.addonButtons[index];
    ...
    public toggleDrawer(selector: string) {
        ...
        if (alwaysShowNoteList && !this.uiElementStates["fallout-maw-calendar-note-list"]) {
            hideExcluded.push("fallout-maw-calendar-note-list");
        }
        ...
        if (cList) {
            const member = selector.toLowerCase() as
                | "fallout-maw-calendar-calendar-list"
                | "fallout-maw-calendar-note-list"
                | "fallout-maw-calendar-note-search"
                | "fallout-maw-calendar-time-options";
            if (alwaysShowNoteList) {
                //If the note list was clicked, and it is not visible (another drawer is) then show the note list
                if (member === "fallout-maw-calendar-note-list" && !this.uiElementStates["fallout-maw-calendar-note-list"]) {
                    this.uiElementStates[member] = animateElement(cList, 500, false);
                }
                //If a drawer was clicked that is not the note list, process.
                else if (member !== "fallout-maw-calendar-note-list") {
                    this.uiElementStates[member] = animateElement(cList, 500, false);
                    //If the drawer is now hidden and the note list is not visible, show the note list.
                    if (!this.uiElementStates[member] && !this.uiElementStates["fallout-maw-calendar-note-list"]) {
                        const noteList = document.querySelector(`#${MainApp.appWindowId} .fallout-maw-calendar-note-list`);
                        if (noteList) {
                            this.uiElementStates["fallout-maw-calendar-note-list"] = animateElement(noteList, 500, false);
                        }
                    }
                }
            } else {
    ...
    public hideDrawers(exclude: string[] = []) {
        document.querySelectorAll(".fallout-maw-calendar-side-drawer").forEach((e) => {
            let hide = true;
            ...
            if (hide) {
                ...
                for (let i = 0; i < this.addonButtons.length; i++) {
                    memberClasses.push(`fallout-maw-calendar-addon-button-side-drawer-${i}`);
                }
    ...
    public toggleUnitSelector(forceHide: boolean = false, event: Event | null = null) {
        event?.stopPropagation();
        const unitList = document.querySelector(`.fallout-maw-calendar-main-wrapper .fallout-maw-calendar-unit-list`);
        if (unitList) {
            ...
            if (this.uiElementStates.compactView) {
                ...
                if (rect.top < window.innerHeight / 2) {
                    unitList.classList.add("fallout-maw-calendar-down");
                } else {
                    unitList.classList.remove("fallout-maw-calendar-down");
                }
            }
        }
    }
    ...
    public changeCalendar(visible: boolean, e: Event) {
        const target = <HTMLElement>e.currentTarget;
        if (target) {
            const wrapper = target.closest(".fallout-maw-calendar-calendar-display");
            if (wrapper) {
                    ...
                    if (!visible && this.activeCalendar.id !== calendarId) {
                        ...
                        if (!GameSettings.IsGm() || !SC.primary) {
                            if (
                                ...
                                ui.notifications?.warn("FALLOUTMAW.Calendar.Warn.Calendar.NotGM", { localize: true });
                            } else {
     ...
     */
    public dayClick(options: FalloutMaWCalendar.Renderer.CalendarOptions) {
        this.toggleUnitSelector(true);
        if (
    ...
    public timeUnitClick(e: Event) {
        ...
        if (dataType && dataAmount) {
            ...
            if (!isNaN(amount)) {
                const interval: FalloutMaWCalendar.DateTimeParts = {};
                if (dataType === "round") {
    ...
    public dateControlApply() {
        if (canUser((<Game>game).user, SC.globalConfiguration.permissions.changeDateTime)) {
            ...
            if (selectedMonth) {
                if (selectedYear !== this.activeCalendar.year.visibleYear || !selectedMonth.visible) {
                    const utsd = new Dialog({
                        title: GameSettings.Localize("FALLOUTMAW.Calendar.SetCurrentDateDialog.Title"),
                        content: GameSettings.Localize("FALLOUTMAW.Calendar.SetCurrentDateDialog.Content").replace(
                            "{DATE}",
                            FormatDateTime(
                                {
                                    year: selectedYear,
            ...
            GameSettings.UiNotification(GameSettings.Localize("FALLOUTMAW.Calendar.Error.Calendar.GMCurrent"), "warn");
        }
    }
     ...
     */
    public searchClick() {
        const searchInput = <HTMLInputElement>document.getElementById("falloutMawCalendarSearchBox");
        if (searchInput) {
    ...
    public searchOptionsToggle(forceClose: boolean = false) {
        const so = document.querySelector(`.fallout-maw-calendar-note-search .fallout-maw-calendar-search-options`);
        if (so) {
    ...
    public searchOptionsFieldsChange(e: Event) {
        ...
        if (element) {
            ...
            if (field && Object.prototype.hasOwnProperty.call(this.search.options.fields, field)) {
                this.search.options.fields[field as keyof FalloutMaWCalendar.Search.OptionsFields] = element.checked;
            }
        }
    }
     ...
     */
    public addNote() {
        NManager.addNewNote(this.visibleCalendar, GameSettings.Localize("FALLOUTMAW.Calendar.Notes.New")).catch(Logger.error);
    }
    ...
    public noteContext(e: Event) {
        const note = <HTMLElement>(<HTMLElement>e.target)?.closest(".fallout-maw-calendar-note");
        const noteID = note?.getAttribute("data-index");
        if (note && noteID) {
            const journalEntry = (<Game>game).journal?.get(noteID);
            if (journalEntry) {
                const sidebarSection = note.closest(".fallout-maw-calendar-section")?.querySelector(":scope > .fallout-maw-calendar-context-menu");
                const appWindow = <HTMLElement>document.getElementById(MainApp.appWindowId);
                if (appWindow && sidebarSection) {
                    sidebarSection.classList.remove("fallout-maw-calendar-hide");
                    ...
                    if (noteStub) {
                        ...
                        sidebarSection
                            .querySelectorAll('div[data-action="edit"], div[data-action="delete"], .fallout-maw-calendar-context-list-break')
                            .forEach((e) => {
                                if (noteStub.canEdit) {
                                    e.classList.remove("fallout-maw-calendar-hide");
                                } else {
                        ...
                        const visibleItems = sidebarSection.querySelectorAll(".fallout-maw-calendar-day-context-list div:not(.fallout-maw-calendar-hide)");
                        ...
                        const reminderAction = sidebarSection.querySelector('.fallout-maw-calendar-context-list-action[data-action="remind"]');
                        if (reminderAction) {
                            reminderAction.innerHTML = `<span class="fa ${noteStub.userReminderRegistered ? "fa-bell-slash" : "fa-bell"}"></span>${
                                noteStub.userReminderRegistered
                                    ? GameSettings.Localize("FALLOUTMAW.Calendar.Notes.ReminderCancel")
                                    : GameSettings.Localize("FALLOUTMAW.Calendar.Notes.Reminder")
                            }`;
                        }
                    }
                }
    ...
    public noteContextClick(e: Event) {
        ...
        if (target && action) {
            const contextMenu = target.closest(".fallout-maw-calendar-context-menu");
    ...
    startTime() {
        if (GameSettings.shouldPauseForCombat()) {
            ui.notifications?.warn("FALLOUTMAW.Calendar.Warn.Time.ActiveCombats", { localize: true });
        } else if (
    ...
    /** Synchronize once at startup without ever jumping an existing world's time. */
    async timeKeepingCheck() {
        if (!SC.primary) return;
        const integration = this.activeCalendar.generalSettings.gameWorldTimeIntegration;
        if (integration === GameWorldTimeIntegrations.Self) {
            await this.activeCalendar.syncTime();
        } else if (integration === GameWorldTimeIntegrations.Mixed || integration === GameWorldTimeIntegrations.ThirdParty) {
            const parsedDate = this.activeCalendar.secondsToDate((<Game>game).time.worldTime);
            this.activeCalendar.setDateTime(parsedDate, {
                updateApp: false,
                sync: false,
                save: false,
