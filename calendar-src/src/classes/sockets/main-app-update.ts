...
export default class MainAppUpdateSocket extends SocketBase {
    ...
    async process(data: FalloutMaWCalendar.FalloutMaWCalendarSocket.Data): Promise<boolean> {
        if (data.type === SocketTypes.mainAppUpdate) {
            const d = <FalloutMaWCalendar.FalloutMaWCalendarSocket.MainAppUpdate>data.data;
            if (d.userId) {
