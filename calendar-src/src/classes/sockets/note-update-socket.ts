...
export default class NoteUpdateSocket extends SocketBase {
    ...
    public async process(data: FalloutMaWCalendar.FalloutMaWCalendarSocket.Data): Promise<boolean> {
        if (data.type === SocketTypes.noteUpdate) {
            const d = <FalloutMaWCalendar.FalloutMaWCalendarSocket.NoteUpdate>data.data;
            ...
            if (je && d.userId) {
                ...
                if (flags) {
                    const userIndex = (<FalloutMaWCalendar.NoteData>flags).remindUsers.indexOf(d.userId);
                    if (userIndex === -1) {
                        (<FalloutMaWCalendar.NoteData>flags).remindUsers.push(d.userId);
                    } else {
                        (<FalloutMaWCalendar.NoteData>flags).remindUsers.splice(userIndex, 1);
                    }
