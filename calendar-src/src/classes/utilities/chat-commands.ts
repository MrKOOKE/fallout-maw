...
export function ParseChatCommand(message: string) {
    ...
    if (messageParts[0] === "/scal" && messageParts.length > 1) {
        ...
        if (processed) {
            ...
            let content = `<div class="fallout-maw-calendar-chat-command-input">${command} ${options}</div>`;
            content += `<div class="fallout-maw-calendar-chat-command-result">${FormatDateTime(
                currentDate,
                `${calendar.generalSettings.dateFormat.date} ${calendar.generalSettings.dateFormat.time}`,
                calendar
            )}</div>`;
            content += `<div class="fallout-maw-calendar-actions"><button class="fallout-maw-calendar-control" data-date='${JSON.stringify({
                year: currentDate.year,
                month: currentDate.month,
                day: currentDate.day
            })}'>${GameSettings.Localize("FALLOUTMAW.Calendar.Chat.ViewInCalendar")}</button></div>`;
            ChatMessage.create({
                content: `<div class="fallout-maw-calendar fallout-maw-calendar-message">${content}</div>`,
