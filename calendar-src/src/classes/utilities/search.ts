...
export class BM25Levenshtein {
     ...
     * @private
     */
    private documents: Record<string, FalloutMaWCalendar.Search.ProcessedDocument> = {};
    /**
     * A list of terms found across all documents
     * @private
     */
    private terms: Record<string, FalloutMaWCalendar.Search.Term> = {};
    /**
    ...
    constructor(documents: FalloutMaWCalendar.Search.Document[]) {
        documents.forEach((d) => {
    ...
    private addDocument(doc: FalloutMaWCalendar.Search.Document) {
        const docContent = Array.isArray(doc.content) ? doc.content.join(" ") : doc.content;
        const docTokens = this.tokenize(docContent);
        const internalDocument: FalloutMaWCalendar.Search.ProcessedDocument = {
            id: doc.id,
