...
import { LocalizeCalendarDisplayAbbreviation, LocalizeCalendarDisplayTerm } from "./display-localization";
 ...
 * @param {*} inputs
 */
export function FormatDateTime(date: FalloutMaWCalendar.DateTime, mask: string, calendar: Calendar, inputs: { year?: boolean; month?: boolean } = {}) {
    inputs = deepMerge({ year: false, month: false }, inputs);
    const token = /DO|d{1,4}|M{1,4}|YN|YA|YZ|YY(?:YY)?|([HhMmsD])\1?|[aA]/g;
    const literal = /\[([^]*?)\]/gm;
    const formatFlags: Record<string, (dateObj: FalloutMaWCalendar.DateTime) => string> = {
        D: (dateObj: FalloutMaWCalendar.DateTime) => {
            return String(calendar.months[dateObj.month]?.days[dateObj.day]?.numericRepresentation || 0);
        },
        DD: (dateObj: FalloutMaWCalendar.DateTime) => {
            return PadNumber(calendar.months[dateObj.month]?.days[dateObj.day]?.numericRepresentation || 0);
        },
        DO: (dateObj: FalloutMaWCalendar.DateTime) => {
        ...
        ddd: (dateObj: FalloutMaWCalendar.DateTime) => {
            const weekday = calendar.weekdays[calendar.dayOfTheWeek(dateObj.year, dateObj.month, dateObj.day)];
            return weekday ? LocalizeCalendarDisplayAbbreviation(weekday.name, weekday.abbreviation) : "";
        },
        dddd: (dateObj: FalloutMaWCalendar.DateTime) => {
            const weekday = calendar.weekdays[calendar.dayOfTheWeek(dateObj.year, dateObj.month, dateObj.day)];
            return weekday ? LocalizeCalendarDisplayTerm(weekday.name) : "";
        },
        M: (dateObj: FalloutMaWCalendar.DateTime) => {
            return String(calendar.months[dateObj.month]?.numericRepresentation || 0);
        },
        MM: (dateObj: FalloutMaWCalendar.DateTime) => {
            return PadNumber(calendar.months[dateObj.month]?.numericRepresentation || 0);
        },
        MMM: (dateObj: FalloutMaWCalendar.DateTime) => {
            const month = calendar.months[dateObj.month];
            return month ? LocalizeCalendarDisplayAbbreviation(month.name, month.abbreviation) : "";
        },
        MMMM: (dateObj: FalloutMaWCalendar.DateTime) => {
            const month = calendar.months[dateObj.month];
            return month ? LocalizeCalendarDisplayTerm(month.name) : "";
        },
        YN: (dateObj: FalloutMaWCalendar.DateTime) => {
        ...
        YY: (dateObj: FalloutMaWCalendar.DateTime) => {
            return inputs.year
                ? `<input type="number" class="fallout-maw-calendar-dont-select" style="width:4ch;" value="${PadNumber(dateObj.year, 2).substring(2)}" />`
                : PadNumber(dateObj.year, 2).substring(2);
        },
        YYYY: (dateObj: FalloutMaWCalendar.DateTime) => {
            return inputs.year
                ? `<input type="number" class="fallout-maw-calendar-dont-select" style="width:${String(dateObj.year).length + 2}ch;" value="${dateObj.year}" />`
                : String(dateObj.year);
        },
        H: (dateObj: FalloutMaWCalendar.DateTime) => {
            return String(dateObj.hour);
        },
        HH: (dateObj: FalloutMaWCalendar.DateTime) => {
        ...
        s: (dateObj: FalloutMaWCalendar.DateTime) => {
            return String(dateObj.seconds);
        },
        ss: (dateObj: FalloutMaWCalendar.DateTime) => {
            return PadNumber(dateObj.seconds);
        },
        A: (dateObj: FalloutMaWCalendar.DateTime) => {
            const halfDay = Math.floor(calendar.time.hoursInDay / 2);
...
export function MergeDateTimeObject(
    ...
    target: FalloutMaWCalendar.DateTime | null = null,
    ...
    return <FalloutMaWCalendar.DateTime>date;
}
...
export function GetDisplayDate(
    calendar: Calendar,
    startDate: FalloutMaWCalendar.DateTime,
    endDate: FalloutMaWCalendar.DateTime,
    allDay: boolean,
 ...
 * @param endDate
 */
export function DateTheSame(startDate: FalloutMaWCalendar.DateTime, endDate: FalloutMaWCalendar.DateTime) {
 ...
 */
export function DaysBetweenDates(calendar: Calendar, startDate: FalloutMaWCalendar.DateTime, endDate: FalloutMaWCalendar.DateTime) {
...
export function IsDayBetweenDates(
    calendar: Calendar,
    checkDate: FalloutMaWCalendar.DateTime,
    startDate: FalloutMaWCalendar.DateTime,
    endDate: FalloutMaWCalendar.DateTime
) {
 ...
 */
export function TimestampToDateData(seconds: number, calendar: Calendar): FalloutMaWCalendar.DateData {
    const result: FalloutMaWCalendar.DateData = {
        year: 0,
        month: 0,
        dayOffset: 0,
        day: 0,
    ...
    const secondOfDay = result.hour * calendar.time.minutesInHour * calendar.time.secondsInMinute
        + result.minute * calendar.time.secondsInMinute
        + result.second;
    const dayStart = seconds - secondOfDay;
    if (calendar.seasons.length) {
        result.sunrise = dayStart + calendar.getSunriseSunsetTime(result.year, result.month, result.day, true, false);
        result.sunset = dayStart + calendar.getSunriseSunsetTime(result.year, result.month, result.day, false, false);
    } else {
        result.sunrise = dayStart + Math.floor(secondsPerDay * 0.25);
        result.sunset = dayStart + Math.floor(secondsPerDay * 0.75);
    }
    result.midday = dayStart + Math.floor(secondsPerDay / 2);
 ...
 * @param calendar The calendar to use to do the conversion
 */
export function DateToTimestamp(date: FalloutMaWCalendar.DateTimeParts, calendar: Calendar): number {
 ...
 */
export function GetPresetTimeOfDay(preset: PresetTimeOfDay, calendar: Calendar, date: FalloutMaWCalendar.Date): FalloutMaWCalendar.Time {
    const time: FalloutMaWCalendar.Time = { hour: 0, minute: 0, seconds: 0 };
    let seconds = 0;
