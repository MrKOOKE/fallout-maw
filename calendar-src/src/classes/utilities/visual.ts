...
export function CheckRemScaling() {
    if (RemSize !== 16) {
        document.body.classList.add(`fallout-maw-calendar-scale-${RemSize}`);
    }
}
...
export function animateElement(element: Element, duration: number, forceHide: boolean = false) {
    let openState = false;
    if (element && !element.classList.contains("fallout-maw-calendar-animate")) {
        if (element.classList.contains("fallout-maw-calendar-open") || forceHide) {
            element.classList.add("fallout-maw-calendar-animate");
            element.classList.remove("fallout-maw-calendar-open");
            openState = false;
            setTimeout(timeoutCall.bind(null, element, true, "fallout-maw-calendar-closed"), duration);
        } else {
            element.classList.add("fallout-maw-calendar-animate", "fallout-maw-calendar-open");
            element.classList.remove("fallout-maw-calendar-closed");
            openState = true;
        }
        setTimeout(timeoutCall.bind(null, element, false, "fallout-maw-calendar-animate"), duration);
    }
    return openState;
}
...
export function animateFormGroup(selector: string, check: boolean, element: Document | Element = document) {
    ...
    if (fg) {
        if ((fg.classList.contains("fallout-maw-calendar-closed") && check) || (fg.classList.contains("fallout-maw-calendar-open") && !check)) {
