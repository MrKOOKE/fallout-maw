...
describe('Utilities Visual Tests', () => {
    ...
    test('Check REM Scaling', () => {
        CheckRemScaling();
        expect(document.body.classList.contains('fallout-maw-calendar-scale-NaN')).toBe(true);
    });

    test('Animate Element', () => {
        let elm = document.createElement('div');
        expect(animateElement(elm, 0)).toBe(true);
        elm.classList.add('fallout-maw-calendar-open');
        elm.classList.remove('fallout-maw-calendar-animate');
    ...
    test('Animate Form Group', () => {
        ...
        expect(elm.classList.contains('fallout-maw-calendar-closed')).toBe(true);
        elm.classList.remove('fallout-maw-calendar-closed');
        animateFormGroup('', true);
        expect(elm.classList.contains('fallout-maw-calendar-open')).toBe(true);
        animateFormGroup('', false);
        elm.classList.remove('fallout-maw-calendar-open');
        elm.classList.add('fallout-maw-calendar-closed');
        animateFormGroup('', true);
    });
});
