...
import { FalloutMaWCalendar } from "../../../types";
...
describe("Utilities Date/Time Tests", () => {
    ...
    test("FormatDateTime", async () => {
        ...
        expect(FormatDateTime({ year: 2021, month: 11, day: 24, hour: 11, minute: 22, seconds: 33 }, "YY YYYY YN YA YZ", tCal, { year: true })).toBe(
            '<input type="number" class="fallout-maw-calendar-dont-select" style="width:4ch;" value="21" /> <input type="number" class="fallout-maw-calendar-dont-select" style="width:6ch;" value="2021" />   '
        );
        //Month
    ...
    test("Get Display Date", async () => {
        const startDate: FalloutMaWCalendar.DateTime = {
            year: 1,
            month: 1,
            day: 10,
            hour: 0,
        ...
        const endDate: FalloutMaWCalendar.DateTime = {
            year: 1,
            month: 1,
            day: 10,
            hour: 0,
    ...
    test("Is Day Between Dates", () => {
        const dateToCheck: FalloutMaWCalendar.DateTime = {
            year: 1,
            month: 1,
            day: 10,
            hour: 0,
    ...
    test("Timestamp to Date", async () => {
        ...
        const pf2eWorldOffset = PF2E.getWorldCreateSeconds(tCal);
        ...
        expect(TimestampToDateData(3600, tCal)).toEqual({
            ...
            hour: 1,
            isLeapYear: false,
            midday: pf2eWorldOffset + 43200,
            minute: 0,
            month: 11,
            second: 0,
            showWeekdayHeadings: true,
            sunrise: pf2eWorldOffset + 21600,
            sunset: pf2eWorldOffset + 64800,
        ...
        expect(TimestampToDateData(5184000, tCal)).toEqual({
            ...
            hour: 0,
            isLeapYear: false,
            midday: pf2eWorldOffset + 5227200,
            minute: 0,
            month: 2,
            second: 0,
            showWeekdayHeadings: true,
            sunrise: pf2eWorldOffset + 5205600,
            sunset: pf2eWorldOffset + 5248800,
            weekdays: ["Moonday", "Toilday", "Wealday", "Oathday", "Fireday", "Starday", "Sunday"],
            year: 4670,
            yearZero: 2700
        });
        expect(TimestampToDateData(5270400, tCal)).toEqual({
            ...
            hour: 0,
            isLeapYear: false,
            midday: pf2eWorldOffset + 5313600,
            minute: 0,
            month: 2,
            second: 0,
            showWeekdayHeadings: true,
            sunrise: pf2eWorldOffset + 5292000,
            sunset: pf2eWorldOffset + 5335200,
            weekdays: ["Moonday", "Toilday", "Wealday", "Oathday", "Fireday", "Starday", "Sunday"],
            year: 4670,
            yearZero: 2700
        });
