...
describe("Chat Timestamp Tests", () => {
    ...
    test("Add Game Time To Message", async () => {
        ...
        expect(cm.flags["fallout-maw"].calendarTimestamps).toEqual({ id: "a", timestamp: tCal.toSeconds() });

        //@ts-ignore
        game.system.id = "D35E";
        //@ts-ignore
        ChatTimestamp.addGameTimeToMessage(cm);
        expect(cm._source.flags["fallout-maw"].calendarTimestamps).toEqual({ id: "a", timestamp: tCal.toSeconds() });
    });
    ...
    test("Update Chat Message Timestamps", () => {
        ...
        const falloutMawCalendarTime = document.createElement("span");
        ...
        // Update SC timestamp
        liqsa.mockReturnValueOnce(fTime).mockReturnValueOnce(falloutMawCalendarTime);
        ChatTimestamp.updateChatMessageTimestamps();
        expect(cqsa).toHaveBeenCalledTimes(3);
        expect(liqsa).toHaveBeenCalledTimes(6);
        expect(fTime.style.display).toBe("none");
        expect(falloutMawCalendarTime.innerText).toBe("test");

        // FTime doesn't exist
        liqsa.mockReturnValueOnce(null).mockReturnValueOnce(falloutMawCalendarTime);
        ChatTimestamp.updateChatMessageTimestamps();
