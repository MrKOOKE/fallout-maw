...
export class ChatTimestamp {
    public static addGameTimeToMessage(chatMessage: ChatMessage) {
        ...
        if (D35E.isD35E) {
            ...
            //@ts-ignore
            chatMessage._source.flags[ModuleName] = { calendarTimestamps: flagData };
        } else {
            ...
            messageFlags[ModuleName].calendarTimestamps = flagData;
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            //@ts-ignore
    ...
    public static getFormattedChatTimestamp(chatMessage: ChatMessage) {
        const timestamps = <{ id: string; timestamp: number }>chatMessage.getFlag(ModuleName, "calendarTimestamps");
        ...
        if (SC.globalConfiguration.inGameChatTimestamp) {
            ...
            if (formattedDateTime) {
                ...
                if (foundryTime) {
                    ...
                    newTime.classList.add("fallout-maw-calendar-timestamp");
        ...
        if (chat) {
            chat.querySelectorAll("#chat-log .message").forEach((li) => {
                ...
                if (message) {
                    ...
                    const stamp = <HTMLElement>li.querySelector(".fallout-maw-calendar-timestamp");
