...
import { PredefinedCalendars, FalloutMaWCalendarHooks } from "../../constants";
...
describe("Hook Tests", () => {
    ...
    test("Emit Date/Time Change", () => {
        Hook.emit(FalloutMaWCalendarHooks.DateTimeChange, tCal);
        ...
        Hook.emit(FalloutMaWCalendarHooks.ClockStartStop, tCal);
        expect(Hooks.callAll).toHaveBeenCalledTimes(4);

        Hook.emit(FalloutMaWCalendarHooks.PrimaryGM, tCal);
