 ...
 *
 * @remarks
 * Simple Calendar exposes a variable called `FalloutMaWCalendar`, all of these API functions exist the property api on that variable `FalloutMaWCalendar.api`
 * @module API
 */
...
/**
 * This function is used to activate event listeners for calendars displayed with the [fallout-maw-calendar-full-calendar](#fallout-maw-calendar-full-calendar) Handlebar helper.
 *
 * If being used in a FoundryVTT application or FormApplication it is best called in the activateListeners function.
 * @param calendarId The ID of the HTML element of the calendar to activate listeners for. This is the same ID used in the [fallout-maw-calendar-full-calendar](#fallout-maw-calendar-full-calendar) Handlebar helper.
...
export function advanceTimeToPreset(preset: PresetTimeOfDay, calendarId: string = "active"): boolean {
    ...
    if (activeCalendar) {
        ...
        Logger.error(`FalloutMaWCalendar.api.advanceTimeToPreset - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
    return false;
}
...
export function changeDate(interval: FalloutMaWCalendar.DateTimeParts, calendarId: string = "active"): boolean {
    ...
    if (activeCalendar) {
        ...
        Logger.error(`FalloutMaWCalendar.api.changeDate - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
    return false;
}
...
export function chooseRandomDate(
    ...
    if (activeCalendar) {
        ...
        Logger.error(`FalloutMaWCalendar.api.chooseRandomDate - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
...
export function clockStatus(calendarId: string = "active"): FalloutMaWCalendar.ClockStatus {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.clockStatus - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        return {
            started: false,
            stopped: false,
            paused: false
...
export async function configureCalendar(
    ...
    if (GameSettings.IsGm()) {
        ...
        if (activeCalendar) {
            ...
            Logger.error(`FalloutMaWCalendar.api.configureCalendar - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        }
    }
    return res;
}
...
export function dateToTimestamp(date: FalloutMaWCalendar.DateTimeParts, calendarId: string = "active"): number {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.dateToTimestamp - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        return 0;
    }
}
...
export function formatDateTime(
    ...
    if (activeCalendar) {
        ...
        Logger.error(`FalloutMaWCalendar.api.formatDateTime - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        return "";
    }
}
...
export function formatTimestamp(timestamp: number, format: string = "", calendarId: string = "active"): string | { date: string; time: string } {
    ...
    if (activeCalendar) {
        ...
        Logger.error(`FalloutMaWCalendar.api.formatTimestamp - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        return "";
    }
}
...
export function getAllCalendars(): FalloutMaWCalendar.CalendarData[] {
    ...
    const data: FalloutMaWCalendar.CalendarData[] = [];
...
export function getAllMonths(calendarId: string = "active"): FalloutMaWCalendar.MonthData[] {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.getAllMonths - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        return [];
    }
}
...
export function getAllMoons(calendarId: string = "active"): FalloutMaWCalendar.MoonData[] {
    ...
    if (activeCalendar) {
        ...
        Logger.error(`FalloutMaWCalendar.api.getAllMoons - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        return [];
    }
}
...
export function getAllSeasons(calendarId: string = "active"): FalloutMaWCalendar.SeasonData[] {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.getAllSeasons - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        return [];
    }
}
...
export function getAllWeekdays(calendarId: string = "active"): FalloutMaWCalendar.WeekdayData[] {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.getAllWeekdays - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        return [];
    }
}
...
export function getCurrentDay(calendarId: string = "active"): FalloutMaWCalendar.DayData | null {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.getCurrentDay - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
    return null;
}
...
export function getCurrentMonth(calendarId: string = "active"): FalloutMaWCalendar.MonthData | null {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.getCurrentMonth - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
    return null;
}
...
export function getCurrentSeason(calendarId: string = "active"): FalloutMaWCalendar.SeasonData {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.getCurrentSeason - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
...
export function getCurrentWeekday(calendarId: string = "active"): FalloutMaWCalendar.WeekdayData | null {
    ...
    if (activeCalendar) {
        ...
        Logger.error(`FalloutMaWCalendar.api.getCurrentWeekday - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
...
export function getCurrentYear(calendarId: string = "active"): FalloutMaWCalendar.YearData | null {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.getCurrentYear - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
    return null;
}
...
export function getLeapYearConfiguration(calendarId: string = "active"): FalloutMaWCalendar.LeapYearData | null {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.getLeapYearConfiguration - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
    return null;
}
...
export function getTimeConfiguration(calendarId: string = "active"): FalloutMaWCalendar.TimeData | null {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.getTimeConfiguration - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
    return null;
}
 ...
 * ```javascript
 *
 * FalloutMaWCalendar.api.isPrimaryGM(); //True or False depending on if the current user is primary gm
 *
 * ```
 */
...
export function pauseClock(calendarId: string = "active"): boolean {
    if (SC.primary) {
        ...
        if (cal) {
            ...
            Logger.error(`FalloutMaWCalendar.api.pauseClock - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        }
    }
    return false;
}
...
export async function removeNote(journalEntryId: string): Promise<boolean> {
    ...
    if (je) {
        ...
        Logger.error(`FalloutMaWCalendar.api.removeNote - Unable to find a journal entry with the passed in ID of "${journalEntryId}"`);
    }
    return false;
}
...
export function searchNotes(
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.searchNotes - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
    return results;
}
...
export function secondsToInterval(seconds: number, calendarId: string = "active"): FalloutMaWCalendar.DateTimeParts {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.secondsToInterval - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
    return {};
}
...
export function setDate(date: FalloutMaWCalendar.DateTimeParts, calendarId: string = "active"): boolean {
    ...
    if (activeCalendar) {
        ...
        Logger.error(`FalloutMaWCalendar.api.setDate - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
    return false;
}
    ...
    if (availableThemes[themeId] !== undefined) {
        ...
        if (themeId !== GetThemeName()) {
            ...
            GameSettings.UiNotification(
                GameSettings.Localize("FALLOUTMAW.Calendar.Info.ThemeChange").replace("{THEME}", GameSettings.Localize(availableThemes[themeId])),
                "info",
                true
            );
        }
        ...
        console.error(`FalloutMaWCalendar.api.setTheme - Unable to find a theme with the ID of "${themeId}"`);
    }
    return false;
}
 ...
 * ```
 */
export function showCalendar(date: FalloutMaWCalendar.DateTimeParts | null = null, compact: boolean = false, calendarId: string = "active"): void {
    const activeCalendar = calendarId === "active" ? CalManager.getActiveCalendar() : CalManager.getCalendar(calendarId);
    ...
    if (activeCalendar) {
        ...
        Logger.error(`FalloutMaWCalendar.api.showCalendar - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
...
export function startClock(calendarId: string = "active"): boolean {
    if (SC.primary) {
        ...
        if (cal) {
            ...
            Logger.error(`FalloutMaWCalendar.api.startClock - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        }
    }
    return false;
}
...
export function stopClock(calendarId: string = "active"): boolean {
    if (SC.primary) {
        ...
        if (cal) {
            ...
            Logger.error(`FalloutMaWCalendar.api.stopClock - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        }
    }
    return false;
}
...
export function timestamp(calendarId: string = "active"): number {
    ...
    if (cal) {
        ...
        Logger.error(`FalloutMaWCalendar.api.timestamp - Unable to find a calendar with the passed in ID of "${calendarId}"`);
    }
    return NaN;
}
 ...
 * ```
 */
export function timestampPlusInterval(currentSeconds: number, interval: FalloutMaWCalendar.DateTimeParts, calendarId: string = "active"): number {
    const activeCalendar = calendarId === "active" ? CalManager.getActiveCalendar() : CalManager.getCalendar(calendarId);
    if (activeCalendar) {
        ...
        Logger.error(`FalloutMaWCalendar.api.timestampPlusInterval - Unable to find a calendar with the passed in ID of "${calendarId}"`);
        return NaN;
    }
}
...
/** Lightweight timestamp conversion used by the system's native calendar bridge. */
export function timestampToDateParts(seconds: number, calendarId: string = "active") {
    const calendar = calendarId === "active" ? CalManager.getActiveCalendar() : CalManager.getCalendar(calendarId);
    if (!calendar) return null;
    if (PF2E.isPF2E && calendar.generalSettings.pf2eSync) seconds += PF2E.getWorldCreateSeconds(calendar);
    const date = calendar.secondsToDate(seconds);
    return {
        year: date.year,
        month: date.month,
        day: date.day,
        ...
        minute: date.minute,
        second: date.seconds,
        dayOfTheWeek: calendar.dayOfTheWeek(date.year, date.month, date.day),
        isLeapYear: calendar.year.leapYearRule.isLeapYear(date.year),
        currentSeason: calendar.getSeason(date.month, date.day + 1)
    };
}

/** Compact O(moons) snapshot for scene-lighting updates. */
export function getMoonLightingState(calendarId: string = "active") {
    const calendar = calendarId === "active" ? CalManager.getActiveCalendar() : CalManager.getCalendar(calendarId);
    if (!calendar) return [];
    const moons = [];
