...
describe('Time Keeper Class Tests', () => {
    ...
    test('Save Interval', () => {
        const tCal = new Calendar('', '');
        jest.spyOn(CalManager, 'getCalendar').mockReturnValue(tCal);
        jest.spyOn(CalManager, 'saveClockTime').mockResolvedValue();
        jest.spyOn(tCal, 'syncTime').mockResolvedValue(true);
        jest.spyOn(GameSockets, 'emit').mockImplementation(async () => {return true;});
        //@ts-ignore
        game.paused = false;
        ...
        expect(CalManager.saveClockTime).not.toHaveBeenCalled();
        ...
        tk.saveInterval();
        expect(CalManager.getCalendar).toHaveBeenCalledTimes(2);
        expect(CalManager.saveClockTime).toHaveBeenCalledTimes(1);
    });
