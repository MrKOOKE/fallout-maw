...
describe('Time Tests', () => {
    ...
    test('Set World Time', async () => {
        const tCal = new Calendar('calendar-id', '');
        const setWorldTime = jest.fn(async () => true);
        jest.spyOn(CalManager, 'getActiveCalendar').mockReturnValue(tCal);
        //@ts-ignore
        game.falloutMaW = {calendar: {setWorldTime}};

        await expect(t.setWorldTime(100)).resolves.toBe(true);
        expect(setWorldTime).toHaveBeenCalledWith(100, {context: {calendarId: tCal.id}});
    });
});
