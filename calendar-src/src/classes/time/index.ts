...
export default class Time extends ConfigurationItemBase {
     ...
     */
    toConfig(): FalloutMaWCalendar.TimeData {
        return {
            id: this.id,
     ...
     */
    loadFromSettings(config: FalloutMaWCalendar.TimeData) {
     ...
     * @return {Time}
     */
    getCurrentTime(): FalloutMaWCalendar.Time {
        let s = this.seconds,
            m = 0,
            h = 0;
    ...
    async setWorldTime(seconds: number) {
        const systemSetWorldTime = (game as any).falloutMaW?.calendar?.setWorldTime;
        if (typeof systemSetWorldTime !== "function") throw new Error("Fallout MaW calendar time bridge is unavailable");
        // Pass the absolute target. The system queue calculates its delta only
        // when this operation reaches the head of the shared mutation queue.
        return Boolean(await systemSetWorldTime(seconds, { context: { calendarId: CalManager.getActiveCalendar().id } }));
    }
}
