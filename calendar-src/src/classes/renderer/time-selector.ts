...
export default class TimeSelector {
    private static defaultOptions: FalloutMaWCalendar.Renderer.TimeSelectorOptions = {
        id: "",
        allowTimeRange: true,
    ...
    public static Render(calendar: Calendar, options: FalloutMaWCalendar.Renderer.TimeSelectorOptions = { id: "" }): string {
        options = deepMerge({}, this.defaultOptions, options);
        let html = `<div id="${options.id}" class="fallout-maw-calendar-time-selector" data-calendar="${CalManager.getAllCalendars(
            options.useCalendarClones
        ).findIndex((c) => {
            return c.id === calendar.id;
        })}">`;
        //Hidden Options
        html += `<input class="fallout-maw-calendar-render-options" type="hidden" value="${encodeURIComponent(
            JSON.stringify(options)
        )}"/><span class="far fa-clock"></span>`;

        if (options.allowTimeRange) {
            html += this.RenderTimeInputGroup(calendar, "fallout-maw-calendar-start-time", options.selectedTime?.start);
            html += `<span class="fallout-maw-calendar-time-spacer">${options.timeDelimiter}</span>`;
            html += this.RenderTimeInputGroup(calendar, "fallout-maw-calendar-end-time", options.selectedTime?.end);
        } else {
    ...
    private static RenderTimeInputGroup(calendar: Calendar, cssClass: string = "", selectedAmount = { hour: 0, minute: 0 }) {
        let html = `<div class="fallout-maw-calendar-ts-inputs ${cssClass}">`;
        //Add Hours
        html += `<input class="fallout-maw-calendar-ts-hour" data-list="fallout-maw-calendar-ts-hour-list" type="number" value="${PadNumber(
            selectedAmount.hour
        )}" /><ul class="fallout-maw-calendar-dropdown fallout-maw-calendar-hide fallout-maw-calendar-ts-hour-list">`;
        ...
        //Ad Minutes
        html += `<span class="fallout-maw-calendar-time-spacer">:</span><input class="fallout-maw-calendar-ts-minute" data-list="fallout-maw-calendar-ts-minute-list" type="number" value="${PadNumber(
            selectedAmount.minute
        )}" /><ul class="fallout-maw-calendar-dropdown fallout-maw-calendar-hide fallout-maw-calendar-ts-minute-list">`;
    ...
    public static ActivateListeners(
        ...
        onTimeChange: ((options: FalloutMaWCalendar.Renderer.TimeSelectorOptions) => void) | null = null
    ) {
        ...
        if (timeSelectorElement) {
            ...
            //Hour Click
            timeSelectorElement.querySelectorAll(".fallout-maw-calendar-ts-inputs .fallout-maw-calendar-ts-hour").forEach((h) => {
                h.addEventListener(
                    "change",
            ...
            //minute Click
            timeSelectorElement.querySelectorAll(".fallout-maw-calendar-ts-inputs .fallout-maw-calendar-ts-minute").forEach((m) => {
                m.addEventListener(
                    "change",
                    TimeSelector.ChangeEventListener.bind(TimeSelector, timeSelectorId, TimeSelectorEvents.change, onTimeChange)
                );
            ...
            //Dropdown Item Click
            timeSelectorElement.querySelectorAll(".fallout-maw-calendar-ts-inputs .fallout-maw-calendar-dropdown").forEach((d) => {
                d.addEventListener(
                    "click",
    ...
    public static HideTimeDropdown(timeSelectorId: string) {
        const timeSelectorElement = document.getElementById(timeSelectorId);
        if (timeSelectorElement) {
            timeSelectorElement.querySelectorAll(".fallout-maw-calendar-dropdown").forEach((n) => {
                n.classList.add("fallout-maw-calendar-hide");
            });
        }
    }
    ...
    public static ChangeEventListener(
        ...
        onTimeChange: ((options: FalloutMaWCalendar.Renderer.TimeSelectorOptions) => void) | null,
        event: Event
    ) {
        ...
        const timeSelectorElement = document.getElementById(timeSelectorId);
        if (timeSelectorElement) {
            let options: FalloutMaWCalendar.Renderer.TimeSelectorOptions = { id: "" };
            const optionsInput = timeSelectorElement.querySelector(".fallout-maw-calendar-render-options");
            if (optionsInput) {
            ...
            if (!isNaN(calendarIndex) && calendarIndex >= 0 && calendarIndex < calendars.length) {
                ...
                if (eventType === TimeSelectorEvents.change || eventType === TimeSelectorEvents.wheel) {
                    ...
                    if (!isNaN(amount)) {
                        isHours = target.classList.contains("fallout-maw-calendar-ts-hour");
                        if (target.parentElement) {
                            isStart = target.parentElement.classList.contains("fallout-maw-calendar-start-time");
                        }
                    } else {
                        amount = 0;
                    }
                    ...
                    if (!isNaN(amount) && target.parentElement) {
                        isHours = target.parentElement.classList.contains("fallout-maw-calendar-ts-hour-list");
                        if (target.parentElement.parentElement) {
                            isStart = target.parentElement.parentElement.classList.contains("fallout-maw-calendar-start-time");
                        }
                    }
                }
                if (options.allowTimeRange) {
    ...
    public static InputClickEventListener(timeSelectorId: string, event: Event) {
        ...
        if (target && target.parentElement) {
            ...
            if (listClass) {
                target.parentElement.querySelector(`.${listClass}`)?.classList.remove("fallout-maw-calendar-hide");
            }
        }
    }
}
