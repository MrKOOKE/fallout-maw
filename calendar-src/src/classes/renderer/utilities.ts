...
export default class RendererUtilities {
    ...
    public static GenerateNoteIconTitle(count: number, notes: NoteStub[]) {
        let rTitle = `${count} ${GameSettings.Localize("FALLOUTMAW.Calendar.Configuration.General.Notes")}`;
        if (notes.length < 3) {
            rTitle = GameSettings.Localize("FALLOUTMAW.Calendar.Configuration.General.Notes") + ":\n";
