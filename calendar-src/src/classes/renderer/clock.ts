...
export default class Clock {
    private static defaultOptions: FalloutMaWCalendar.Renderer.ClockOptions = {
        id: "",
        cssClasses: "",
        theme: "auto"
    };
     ...
     * @param options
     */
    public static Render(calendar: Calendar, options: FalloutMaWCalendar.Renderer.ClockOptions = { id: "" }): string {
        options = deepMerge({}, this.defaultOptions, options);
        const status = calendar.timeKeeper.getStatus();
        options.cssClasses += ` ${status}`;

        let html = `<div id="${options.id}" class="fallout-maw-calendar-clock ${options.cssClasses} ${
            status === TimeKeeperStatus.Started ? "fallout-maw-calendar-animate" : ""
        ...
        html += `<div class="fallout-maw-calendar-animated-clock">${GetIcon(Icons.Clock)}</div>`;
    ...
    public static RenderTime(calendar: Calendar) {
        return `<div class="fallout-maw-calendar-current-time">${calendar.time.toString()}</div>`;
    }
    ...
    public static UpdateListener(clockId: string, status: TimeKeeperStatus) {
        ...
        if (clockElement) {
            ...
            const calendar = CalManager.getCalendar(calendarIndex);
            if (calendar) {
                /*let options: FalloutMaWCalendar.Renderer.ClockOptions = { id: "" };
                const optionsInput = clockElement.querySelector(".fallout-maw-calendar-render-options");
                if (optionsInput) {
                ...
                if (temp.firstChild) {
                    const timeElement = clockElement.querySelector(".fallout-maw-calendar-current-time");
                    if (timeElement) {
                        timeElement.replaceWith(temp.firstChild);
                        if (!clockElement.classList.contains(status)) {
                            clockElement.classList.remove(TimeKeeperStatus.Started, TimeKeeperStatus.Stopped, TimeKeeperStatus.Paused, "fallout-maw-calendar-animate");
                            clockElement.classList.add(status);
                            if (status === TimeKeeperStatus.Started) {
                                clockElement.classList.add("fallout-maw-calendar-animate");
                            }
                        }
                    }
                }
