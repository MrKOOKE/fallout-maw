...
describe("Renderer Clock Class Tests", () => {
    ...
    test("Render", () => {
        ...
        expect(html).toContain('<div class="fallout-maw-calendar-current-time">00:00:00</div>');
        expect(html).not.toContain('fallout-maw-calendar-animate"');

        //@ts-ignore
        tCal.timeKeeper.status = TimeKeeperStatus.Started;
        html = Clock.Render(tCal);
        expect(html).toContain('<div class="fallout-maw-calendar-current-time">00:00:00</div>');
        expect(html).toContain('fallout-maw-calendar-animate"');
    ...
    test("Render Time", () => {
        expect(Clock.RenderTime(tCal)).toContain('<div class="fallout-maw-calendar-current-time">00:00:00</div>');
    });
