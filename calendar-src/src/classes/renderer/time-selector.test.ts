...
describe('Renderer Time Selector Class Tests', () => {
    ...
    test('Render', () => {
        expect(TimeSelector.Render(tCal)).toContain('fallout-maw-calendar-start-time');
        expect(TimeSelector.Render(tCal, {id:'', allowTimeRange: false})).not.toContain('fallout-maw-calendar-start-time');
    });

    test('Render Time Input Group', () => {
        //@ts-ignore
        expect(TimeSelector.RenderTimeInputGroup(tCal)).toContain('fallout-maw-calendar-ts-inputs');
    });
    ...
    test('Hide Time Dropdown', () => {
        ...
        expect(cElm.classList.contains('fallout-maw-calendar-hide')).toBe(true);
    });
    ...
    test('Change Event Listener', () => {
        ...
        target.value = '-1';
        target.classList.add('fallout-maw-calendar-ts-hour');
        targetParent.classList.add('fallout-maw-calendar-start-time');
        //@ts-ignore
        ...
        target.classList.remove('fallout-maw-calendar-ts-hour');
        //@ts-ignore
        ...
        target.classList.add('fallout-maw-calendar-ts-hour');
        targetParent.classList.remove('fallout-maw-calendar-start-time');
        //@ts-ignore
    ...
    test('Input Click Event Listener', () => {
        ...
        const cElm = document.createElement('div');
        tsParent.append(tsElm);
        cElm.classList.add('fallout-maw-calendar-hide');
        ...
        expect(cElm.classList.contains('fallout-maw-calendar-hide')).toBe(false);
    });
});
