...
describe('Note Stub Class Tests', () => {
    ...
    test('Author Display', () => {
        ...
        jest.spyOn((<Game>game).i18n!, "localize").mockReturnValue("System");
    ...
    test('Player Visible', () => {
        ...
        expect(nStub.playerVisible).toEqual({ icon: 'fa-eye-slash', color: 'fallout-maw-calendar-danger', players: "" });
        //@ts-ignore
        jest.spyOn(game.users, 'filter').mockReturnValueOnce([]);
        expect(nStub.playerVisible).toEqual({ icon: 'fa-eye', color: 'fallout-maw-calendar-success', players: ":<ul style=\"text-align: left;padding: 0;margin-bottom:0;list-style: none;\"><li></li></ul>" });
        //@ts-ignore
        jest.spyOn(game.users, 'filter').mockReturnValue([game.user, game.user]);
        //@ts-ignore
        (<Mock>game.users.get).mockReturnValueOnce(game.user);
        expect(nStub.playerVisible).toEqual({ icon: 'fa-eye-low-vision', color: 'fallout-maw-calendar-secondary', players: ":<ul style=\"text-align: left;padding: 0;margin-bottom:0;list-style: none;\"><li></li><li></li></ul>" });
    });
