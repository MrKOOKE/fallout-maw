...
export default class NoteStub {
        ...
        if (noteData && note.userReminderRegistered) {
            ...
            if (calendar && (!initialLoad || (initialLoad && calendar.generalSettings.postNoteRemindersOnFoundryLoad))) {
                if (SC.clientSettings.noteReminderNotification === NoteReminderNotificationType.whisper) {
                    ChatMessage.create({
                        speaker: { alias: GameSettings.Localize("FALLOUTMAW.Calendar.Notes.ReminderSpeaker") },
    ...
    public get noteData() {
        ...
        if (journalEntry) {
            return <FalloutMaWCalendar.NoteData>journalEntry.getFlag(ModuleName, "noteData");
        }
        return null;
    }
    ...
    public get authorDisplay() {
        ...
        if (journalEntry) {
            ...
            if (noteData && noteData.fromPredefined) {
                return {
                    name: GameSettings.Localize("FALLOUTMAW.Calendar.Notes.SystemAuthor"),
                    color: "",
                    colorText: ""
                };
            } else {
     ...
     */
    public get categories(): FalloutMaWCalendar.NoteCategory[] {
        const noteData = this.noteData;
        if (noteData) {
            const calendar = CalManager.getCalendar(noteData.calendarId);
            if (calendar) {
    ...
    public get playerVisible() {
        ...
        const result = {
            icon: "fa-eye-slash",
            color: "fallout-maw-calendar-danger",
            players: ""
        };
        if (journalEntry) {
            const own = this.ownership;
            ...
            if (playerCoverage) {
                if (playerCoverage.length === (<Game>game).users?.contents.length) {
                    result.icon = "fa-eye";
                    result.color = "fallout-maw-calendar-success";
                } else if (playerCoverage.length > 1) {
                    result.icon = "fa-eye-low-vision";
                    result.color = "fallout-maw-calendar-secondary";
                }
                result.players = `${GameSettings.Localize(
                    "FALLOUTMAW.Calendar.Notes.UserPermissionTitle"
