...
export default class NoteManager {
    ...
    public registerNoteSheets() {
        foundry.documents.collections.Journal.registerSheet(ModuleName, NoteSheet, { 
            types: ["base"], 
            makeDefault: false, 
            label: GameSettings.Localize("FALLOUTMAW.Calendar.Notes.SheetLabel")
        });
    }
    ...
    public async createJournalDirectory() {
        this.noteDirectory = game.folders?.find((f) => f.type === "JournalEntry" && !!f.getFlag(ModuleName, "calendarRoot"));
    ...
    public async addNewNote(calendar: Calendar, title: string) {
        ...
        const noteData: FalloutMaWCalendar.NoteData = {
            ...
            await newJE.createEmbeddedDocuments("JournalEntryPage", [
                {
                    ...
                    name: GameSettings.Localize("FALLOUTMAW.Calendar.Details")
                }
            ]);
            if (renderSheet) {
    ...
    public journalEntryUpdate(type: number, entry: JournalEntry) {
        const noteData = <FalloutMaWCalendar.NoteData>entry.getFlag(ModuleName, "noteData");
    ...
    public removeNoteStub(journalEntry: JournalEntry) {
        const noteData = <FalloutMaWCalendar.NoteData>journalEntry.getFlag(ModuleName, "noteData");
    ...
    private async loadNotesFromFolder(folder: Folder) {
        for (let i = 0; i < folder.contents.length; i++) {
            ...
            const noteData = <FalloutMaWCalendar.NoteData>je.getFlag(ModuleName, "noteData");
            if (noteData) {
     ...
     */
    public async orderNotesOnDay(calendarId: string, newOrderedIds: string[], day: FalloutMaWCalendar.DateTime) {
     ...
     * @param options
     */
    public searchNotes(calendarId: string, term: string, options: FalloutMaWCalendar.Search.OptionsFields): NoteStub[] {
        const noteList = this.notes[calendarId];
        let results: NoteStub[] = [];
        if (noteList) {
            //prepare the note list for searching
            const nl: FalloutMaWCalendar.Search.Document[] = [];
            for (let i = 0; i < noteList.length; i++) {
