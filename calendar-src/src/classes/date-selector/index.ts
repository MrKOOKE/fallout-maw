...
export class DateSelector {
     ...
     */
    selectedDate: FalloutMaWCalendar.DateTimeSelector.Dates;
    /**
     * A function to call when a date is selected
     */
    onDateSelect: ((selectedDates: FalloutMaWCalendar.DateTimeSelector.SelectedDates) => void) | null = null;
    /**
     ...
     */
    constructor(id: string, options: FalloutMaWCalendar.DateTimeSelector.Options) {
        this.id = id;
     ...
     * @param options
     */
    applyOptions(options: FalloutMaWCalendar.DateTimeSelector.Options) {
    ...
    build(hideCalendar = true, includeWrapper: boolean = true) {
        let returnHtml;
        const wrapper = `<div id="${this.id}" class="fallout-maw-calendar-date-selector">`;
        let calendar = "";
        let timeSelectors = "";
        ...
        if (this.showDateSelector) {
            calendar = Renderer.CalendarFull.Render(this.calendar, {
                ...
                cssClasses: "fallout-maw-calendar-date-selector-calendar",
        ...
        if (this.showTimeSelector) {
            timeSelectors = `<div class="fallout-maw-calendar-date-selector-time-selector-wrapper">`;
            if (this.addTime) {
                ...
                if (this.showDateSelector) {
                    timeSelectors += `<div class="fallout-maw-calendar-remove-time"><button class="fallout-maw-calendar-control fallout-maw-calendar-delete"><i class="fa fa-times"></i> ${GameSettings.Localize(
                        "FALLOUTMAW.Calendar.RemoveTime"
                    )}</button></div>`;
                }
            } else {
                timeSelectors += `<div class="fallout-maw-calendar-add-time"><button class="fallout-maw-calendar-control fallout-maw-calendar-primary"><i class="fa fa-clock"></i> ${GameSettings.Localize(
                    "FALLOUTMAW.Calendar.Notes.DateTime.AllDay"
                )}</button></div>`;
            }
            timeSelectors += `</div>`;
        }
        let selectedEndDate: FalloutMaWCalendar.DateTime = {
            year: this.selectedDate.end.year,
            month: this.selectedDate.end.month,
            day: this.selectedDate.end.day,
            hour: this.selectedDate.end.hour,
        ...
        returnHtml = `<input class="fallout-maw-calendar-display-input" value="${displayDate}" tabindex="0" type="text" readonly="readonly"><div class="fallout-maw-calendar-date-selector-calendar-wrapper" style="display:${
    ...
    update(hideCalendar: boolean = false) {
        ...
        if (ds) {
            ...
            this.setPosition(<HTMLElement>ds.querySelector(".fallout-maw-calendar-date-selector-calendar-wrapper"));
        }
    }
    ...
    activateListeners(
        ...
        if (html) {
            ...
            if (activateCalendarListeners && activateTimeListeners) {
                const di = <HTMLElement>html.querySelector(".fallout-maw-calendar-display-input");
                if (di) {
            ...
            if (this.showDateSelector) {
                const cal = <HTMLElement>html.querySelector(".fallout-maw-calendar-date-selector-calendar");
                if (cal) {
            ...
            if (this.showTimeSelector) {
                const atb = html.querySelector(".fallout-maw-calendar-add-time button");
                if (atb) {
                    atb.addEventListener("click", this.addTimeClick.bind(this));
                }
                const cb = html.querySelector(".fallout-maw-calendar-remove-time button");
                if (cb) {
    ...
    callOnDateSelect() {
        if (this.onDateSelect) {
            ...
            const returnValue: FalloutMaWCalendar.DateTimeSelector.SelectedDates = {
                startDate: {
                    year: this.selectedDate.start.year,
                    month: this.selectedDate.start.month,
                    day: this.selectedDate.start.day,
    ...
    toggleCalendar(html: HTMLElement, event: Event) {
        ...
        const cal = <HTMLElement>html.querySelector(".fallout-maw-calendar-date-selector-calendar-wrapper");
        if (cal) {
    ...
    calendarClick(event: Event) {
        ...
        if (wrapper) {
            const dropdowns = wrapper.getElementsByClassName("fallout-maw-calendar-time-dropdown");
     ...
     */
    dayClick(options: FalloutMaWCalendar.Renderer.CalendarOptions) {
     ...
     * @param options
     */
    changeMonthClick(clickType: CalendarClickEvents, options: FalloutMaWCalendar.Renderer.CalendarOptions) {
        if (options.date) {
     ...
     * @param options
     */
    changeYear(options: FalloutMaWCalendar.Renderer.CalendarOptions) {
        if (options.date) {
            this.selectedDate.visible.year = options.date.year;
        }
        this.update(false);
     ...
     * @param options
     */
    timeChange(options: FalloutMaWCalendar.Renderer.TimeSelectorOptions) {
