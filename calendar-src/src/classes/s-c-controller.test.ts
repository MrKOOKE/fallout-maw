...
describe("SCController Tests", () => {
    ...
    test("Properties", () => {
        expect(Object.keys(SC).length).toBe(3); //Make sure no new properties have been added
    });
    ...
    test("Persistence Change", () => {
        ...
        expect(mainApp.classList.contains("fallout-maw-calendar-persistent")).toBe(false);
        SC.PersistenceChange();
        expect(mainApp.classList.contains("fallout-maw-calendar-persistent")).toBe(true);
    });
    ...
    test("Compact Scale Change", () => {
        ...
        expect(mainApp.classList.contains("fallout-maw-calendar-scale-120")).toBe(true);
        SC.CompactScaleChange();
        expect(mainApp.classList.contains("fallout-maw-calendar-scale-120")).toBe(false);
        expect(mainApp.classList.contains("fallout-maw-calendar-scale-200")).toBe(true);
    });
    ...
    test("Hide Context Menus", () => {
        ...
        context.classList.add("fallout-maw-calendar-context-menu");
        document.body.append(context);
        SCController.HideContextMenus();
        expect(context.classList.contains("fallout-maw-calendar-hide")).toBe(true);
    });
    ...
    test("Render Scene Controls", () => {
        const canUserSpy = jest.spyOn(PermUtils, "canUser").mockReturnValue(true);
        const clickSpy = jest.spyOn(MainApplication, "sceneControlButtonClick").mockImplementation(() => {});
        const root = document.createElement("div");
        root.id = "scene-controls-layers";
        const menu = document.createElement("menu");
        const notesItem = document.createElement("li");
        const notesButton = document.createElement("button");
        notesButton.dataset.control = "notes";
        notesItem.append(notesButton);
        menu.append(notesItem);
        root.append(menu);

        canUserSpy.mockReturnValueOnce(false);
        SC.renderSceneControls({}, root);
        expect(menu.querySelector("[data-fallout-maw-calendar-control]")).toBeNull();

        SC.renderSceneControls({}, root);
        const calendarButton = menu.querySelector<HTMLButtonElement>("[data-fallout-maw-calendar-control]");
        expect(calendarButton).not.toBeNull();
        expect(notesItem.nextElementSibling).toBe(calendarButton?.closest("li"));
        calendarButton?.click();
        expect(clickSpy).toHaveBeenCalledTimes(1);

        SC.renderSceneControls({}, root);
        expect(menu.querySelectorAll("[data-fallout-maw-calendar-control]")).toHaveLength(1);
    });
    ...
    test("World Time Update", () => {
        const tCal = new Calendar("", "");
        const setFromTimeSpy = jest.spyOn(tCal, "setFromTime").mockImplementation(() => {});
        jest.spyOn(CalManager, "getActiveCalendar").mockReturnValue(tCal);
        const options = { worldTime: { delta: 10 } };
        SC.worldTimeUpdate(100, 10, options);
        expect(setFromTimeSpy).toHaveBeenCalledWith(100, 10, options);
    });
    ...
    test("Combat Update", () => {
        const tCal = new Calendar("", "");
        jest.spyOn(CalManager, "getActiveCalendar").mockReturnValue(tCal);
        const processOwnCombatRoundTimeSpy = jest.spyOn(tCal, "processOwnCombatRoundTime").mockImplementation(() => {});
        ...
        expect(tCal.time.combatRunning).toBe(true);
        //@ts-ignore
        SC.combatUpdate({ started: true, scene: { id: "123" } }, {}, { worldTime: { delta: 2 } });
        expect(tCal.combatChangeTriggered).toBe(true);
        //@ts-ignore
        SC.combatUpdate({ started: true, scene: { id: "123" } }, {}, { worldTime: { delta: 0 } });
        expect(tCal.combatChangeTriggered).toBe(true);
        expect(processOwnCombatRoundTimeSpy).toHaveBeenCalledTimes(1);
