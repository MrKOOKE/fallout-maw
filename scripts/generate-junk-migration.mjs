...
const MACRO_OUTPUT_DIR = path.join(systemRoot, "scripts", "migration-macros", "junk");
const MACRO_FILE = "01-import-junk-items.js";
const ROOT_FOLDER = "MAW Импорт хлама";
const JUNK_FOLDER_PREFIX = "Хлам / ";
...
async function main() {
  ...
  const candidates = Array.from(itemById.values())
    ...
    .filter(entry => entry.folderPath.startsWith(JUNK_FOLDER_PREFIX))
    .sort((left, right) => (
  ...
  for (const entry of candidates) {
    records.push(await createJunkMigrationRecord(entry.item, entry.folderPath, itemById));
  }
  ...
  await fs.writeFile(
    ...
    buildJunkMacro(records, craftReferenceNames, buildStamp),
    "utf8"
  );
  await fs.writeFile(
    path.join(MACRO_OUTPUT_DIR, "README.md"),
    buildReadme(records.length, buildStamp),
    "utf8"
  );
  await fs.writeFile(
  ...
  console.log(`Junk items found: ${candidates.length}`);
...
async function createJunkMigrationRecord(item, folderPath, itemById) {
  ...
  return {
    ...
    name: item.name,
    img,
    folderPath: normalizeJunkFolderPath(folderPath).split(" / ").filter(Boolean),
    oldType: item.type,
    ...
    system: {
      ...
      maxStack: 5,
      itemCategory: getOldItemSection(item) || "Хлам",
...
function normalizeJunkFolderPath(folderPath) {
  return String(folderPath ?? "").replace(/^Хлам \/ /, "");
}
...
function buildJunkMacro(records, craftReferenceNames, buildStamp) {
  return `// Generated by systems/fallout-maw/scripts/generate-junk-migration.mjs
// ${buildStamp}

const JUNK_ITEMS = ${JSON.stringify(records, null, 2)};

const SYSTEM_ID = "fallout-maw";
const ROOT_FOLDER = ${JSON.stringify(ROOT_FOLDER)};
const FLAG_SCOPE = "fallout-maw";
const FLAG_KEY = "junkMigration";
...
async function runJunkImport() {
  ...
  if (missingReferences.size) {
    ui.notifications.warn(\`Импорт хлама: обработано \${touched.length}. Не удалось разрешить \${missingReferences.size} ссылок крафта.\`);
    console.warn("Junk import unresolved craft references", Array.from(missingReferences));
  } else {
    ui.notifications.info(\`Импорт хлама: обработано \${touched.length}. Все ссылки крафта разрешены.\`);
  }
  console.log("Junk import", { touched: touched.length, missingReferences: Array.from(missingReferences) });
}
...
function resolveImportedItemId(oldId, idMap, hintName) {
  ...
  const byJunkFlag = game.items.find(item => item.getFlag(FLAG_SCOPE, FLAG_KEY)?.oldId === resolvedOldId)
...
function buildReadme(itemCount, buildStamp) {
  return `# Макросы миграции хлама
...
- \`${MACRO_FILE}\` — ${itemCount} предметов из папки \`Хлам / …\` старого мира
...
function buildPasteMacro() {
  return `// Fallout-MaW junk migration: one Foundry macro.
...
const JUNK_IMPORT_FILE = "${MACRO_FILE}";
