...
import {
  ...
  normalizeWeaponLimbDamageMultipliers,
...
const STANDARD_WEAPON_LIMB_KEYS = Object.freeze([
  "head",
  "eyes",
  "torso",
  "groin",
...
export class FalloutMaWItemSheet extends HandlebarsApplicationMixin(ItemSheetV2) {
  ...
  async _prepareContext(options) {
    ...
    const itemSubcategory = item.system?.itemSubcategory ?? "";
    const itemCategories = getItemCategorySettings();
    const itemSubcategoryChoices = buildItemSubcategoryChoices(itemCategory, itemSubcategory, itemCategories);
    ...
    const proficiencySettings = getProficiencySettings(rulesProfile);
    ...
    const abilityFunctionChoices = [
      ...
      || choice.value !== ABILITY_FUNCTION_TYPES.fixed
    ));
    ...
    return foundry.utils.mergeObject(context, {
      ...
      showFixedAbilityFunctionPicker: rulesProfile.fixedAbilityFunctionsEnabled !== false
        && this.#fixedAbilityFunctionPickerActive,
      ...
      abilityConstructs: type === "ability"
        ? normalizeAbilityConstructs(item.system?.constructs).map(prepareItemAbilityConstructForDisplay)
        : [],
  ...
  _processSubmitData(event, form, submitData, options = {}) {
    ...
    normalizeSubmittedAbilityConstructDurations(form, submitData);
    ...
    normalizeSubmittedItemSubcategory(submitData, this.item);
  ...
  async _onRender(context, options) {
    ...
    this.element?.querySelectorAll("[data-fixed-inconspicuous-attacked]").forEach(select => {
      this.#addHandledFormChangeListener(select, event => this.#onInconspicuousRoundStatusChange(event));
    });
    ...
    this.element?.querySelectorAll("[data-delete-ability-action]").forEach(button => {
      button.addEventListener("click", event => this.#onDeleteAbilityAction(event));
    });
    this.element?.querySelectorAll("[data-add-ability-attack-choice]").forEach(button => {
      button.addEventListener("click", event => this.#onAddAbilityAttackChoice(event));
    });
    this.element?.querySelectorAll("[data-delete-ability-attack-choice]").forEach(button => {
      button.addEventListener("click", event => this.#onDeleteAbilityAttackChoice(event));
    });
    this.element?.querySelectorAll("[data-ability-action-cost-mode]").forEach(select => {
      select.addEventListener("change", () => syncItemAbilityActionCostVisibility(select));
      syncItemAbilityActionCostVisibility(select);
    });
    this.element?.querySelectorAll("[data-ability-attack-choice]").forEach(select => {
      select.addEventListener("change", () => syncItemAbilityAttackChoiceControls(select));
      syncItemAbilityAttackChoiceControls(select);
    });
    ...
    this.element?.querySelectorAll("[data-add-ability-trial-entry]").forEach(button => {
      button.addEventListener("click", event => this.#onAddAbilityTrialEntry(event));
    });
    ...
    this.element?.querySelectorAll("[data-delete-ability-trial-link]").forEach(button => {
      button.addEventListener("click", event => this.#onDeleteAbilityTrialLink(event));
    });
    this.element?.querySelectorAll("[data-ability-trial-link-type]").forEach(select => {
      this.#addHandledFormChangeListener(select, event => this.#onAbilityTrialLinkTypeChange(event));
    });
    ...
    this.element?.querySelectorAll("[data-delete-ability-construct]").forEach(button => {
      button.addEventListener("click", event => this.#onDeleteAbilityConstruct(event));
    });
    this.element?.querySelectorAll("[data-add-ability-construct-change]").forEach(button => {
      button.addEventListener("click", event => this.#onAddAbilityConstructChange(event));
    });
    this.element?.querySelectorAll("[data-delete-ability-construct-change]").forEach(button => {
      button.addEventListener("click", event => this.#onDeleteAbilityConstructChange(event));
    });
    this.element?.querySelectorAll("[data-add-ability-construct-resource]").forEach(button => {
      button.addEventListener("click", event => this.#onAddAbilityConstructResource(event));
    });
    this.element?.querySelectorAll("[data-delete-ability-construct-resource]").forEach(button => {
      button.addEventListener("click", event => this.#onDeleteAbilityConstructResource(event));
    });
    this.element?.querySelectorAll("[data-add-ability-trigger-cost]").forEach(button => {
      button.addEventListener("click", event => this.#onAddAbilityTriggerCost(event));
    });
    ...
    this.element?.querySelectorAll("[data-add-ability-event-expected-result]").forEach(button => {
      button.addEventListener("click", event => this.#onAddAbilityEventExpectedResult(event));
    });
    this.element?.querySelectorAll("[data-delete-ability-event-expected-result]").forEach(button => {
      button.addEventListener("click", event => this.#onDeleteAbilityEventExpectedResult(event));
    });
    this.element?.querySelectorAll("[data-add-ability-event-depth-filter]").forEach(button => {
      button.addEventListener("click", event => this.#onAddAbilityEventDepthFilter(event));
    });
    ...
    this.element?.querySelectorAll("[data-add-ability-region-damage-type]").forEach(button => {
      button.addEventListener("click", event => this.#onAddAbilityRegionDamageType(event));
    });
    this.element?.querySelectorAll("[data-delete-ability-region-damage-type]").forEach(button => {
      button.addEventListener("click", event => this.#onDeleteAbilityRegionDamageType(event));
    });
    this.element?.querySelectorAll("[data-add-ability-region-special-property]").forEach(button => {
      button.addEventListener("click", event => this.#onAddAbilityRegionSpecialProperty(event));
    });
    this.element?.querySelectorAll("[data-delete-ability-region-special-property]").forEach(button => {
      button.addEventListener("click", event => this.#onDeleteAbilityRegionSpecialProperty(event));
    });
    ...
    this.element?.querySelectorAll("[data-add-weapon-additional-proficiency]").forEach(button => {
      button.addEventListener("click", event => this.#onAddWeaponAdditionalProficiency(event));
    });
    this.element?.querySelectorAll("[data-delete-weapon-additional-proficiency]").forEach(button => {
      button.addEventListener("click", event => this.#onDeleteWeaponAdditionalProficiency(event));
    });
  ...
  async #onInconspicuousRoundStatusChange(event) {
    const select = event.currentTarget;
    const functionId = String(select?.closest?.("[data-ability-function-row]")?.dataset.functionId ?? "");
    const abilityFunction = normalizeAbilityFunctions(this.item?.system?.functions ?? [])
      .find(entry => (
        entry.id === functionId
        && entry.fixedKey === ABILITY_FIXED_FUNCTION_KEYS.inconspicuous
      ));
    ...
    return this.item.update(buildInconspicuousRoundStateUpdate(abilityFunction, {
      combat,
      attacked: String(select?.value ?? "false") === "true"
    }));
  }
  ...
  #onAddAbilityAction(event) {
    ...
    const functions = this.#getSubmittedAbilityFunctions(functionPath);
    const entry = functions[functionIndex];
    if (![ABILITY_FUNCTION_TYPES.effectChanges, ABILITY_FUNCTION_TYPES.activeApplication].includes(entry?.type)) return undefined;
    entry.actions ??= [];
    entry.actions.push(createAbilityAction());
    return this.#submitCurrentForm({ [functionPath]: functions });
  }

  #onDeleteAbilityAction(event) {
    event.preventDefault();
    event.stopPropagation();
    const functionIndex = Number(event.currentTarget?.closest?.("[data-ability-function-row]")?.dataset.functionIndex ?? -1);
    const actionIndex = Number(event.currentTarget?.closest?.("[data-ability-action-row]")?.dataset.actionIndex ?? -1);
    const functionPath = this.#getAbilityFunctionPathForEvent(event);
    const functions = this.#getSubmittedAbilityFunctions(functionPath);
    if (!functions[functionIndex]?.actions?.[actionIndex]) return undefined;
    functions[functionIndex].actions.splice(actionIndex, 1);
    return this.#submitCurrentForm({ [functionPath]: functions });
  }

  #onAddAbilityAttackChoice(event) {
    event.preventDefault();
    const functionIndex = Number(event.currentTarget?.closest?.("[data-ability-function-row]")?.dataset.functionIndex ?? -1);
    const actionIndex = Number(event.currentTarget?.closest?.("[data-ability-action-row]")?.dataset.actionIndex ?? -1);
    const functionPath = this.#getAbilityFunctionPathForEvent(event);
    const functions = this.#getSubmittedAbilityFunctions(functionPath);
    const action = functions[functionIndex]?.actions?.[actionIndex];
  ...
  #onDeleteAbilityAttackChoice(event) {
    event.preventDefault();
    event.stopPropagation();
    const functionIndex = Number(event.currentTarget?.closest?.("[data-ability-function-row]")?.dataset.functionIndex ?? -1);
    const actionIndex = Number(event.currentTarget?.closest?.("[data-ability-action-row]")?.dataset.actionIndex ?? -1);
    const choiceIndex = Number(event.currentTarget?.closest?.("[data-ability-attack-choice-row]")?.dataset.choiceIndex ?? -1);
    const functionPath = this.#getAbilityFunctionPathForEvent(event);
    const functions = this.#getSubmittedAbilityFunctions(functionPath);
    const choices = functions[functionIndex]?.actions?.[actionIndex]?.attackActionKeys;
    if (!Array.isArray(choices) || choices.length <= 1 || choiceIndex < 0) return undefined;
  ...
  #onDeleteAbilityTrialEntry(event) {
    event.preventDefault();
    event.stopPropagation();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    const entryIndex = Number(
      event.currentTarget?.closest?.("[data-ability-trial-entry-row]")?.dataset.trialEntryIndex ?? -1
    );
    if (condition?.type !== ABILITY_CONDITION_TYPES.trial || entryIndex < 0) return undefined;
  ...
  async #onAbilityTrialLinkTypeChange(event) {
    ...
    if (condition?.type !== ABILITY_CONDITION_TYPES.trial || !link) return undefined;
    ...
    if ([
      ...
      const existing = constructs.find(construct => String(construct?.id ?? "") === previousConstructId);
      if (!existing || existing.type !== selectedType) {
        const construct = prepareNewItemOrdinaryTrialConstruct(createAbilityConstruct(selectedType));
    ...
    const nextConstructs = previousConstructId
  ...
  #onDeleteAbilityTrialLink(event) {
    ...
    const nextConstructs = constructId && !isAbilityConstructLinkedInFunctions(functions, constructId)
      ? constructs.filter(construct => String(construct?.id ?? "") !== constructId)
      : constructs;
    return this.#submitCurrentForm({
      [functionPath]: functions,
      "system.constructs": nextConstructs
  ...
  #onAddAbilityConstruct(event) {
    ...
    const constructs = this.#getSubmittedAbilityConstructs();
    constructs.push(createAbilityConstruct(type));
    return this.#submitCurrentForm({ "system.constructs": constructs });
  }

  #onDeleteAbilityConstruct(event) {
    event.preventDefault();
    event.stopPropagation();
    const constructIndex = Number(
      event.currentTarget?.closest?.("[data-ability-construct-row]")?.dataset.constructIndex ?? -1
    );
    const constructs = this.#getSubmittedAbilityConstructs();
    if (constructIndex < 0 || !constructs[constructIndex]) return undefined;
    const removedId = constructs[constructIndex].id;
    constructs.splice(constructIndex, 1);
    const functions = this.#getSubmittedAbilityFunctions("system.functions");
  ...
  #onAddAbilityConstructChange(event) {
    event.preventDefault();
    const constructIndex = Number(
      event.currentTarget?.closest?.("[data-ability-construct-row]")?.dataset.constructIndex ?? -1
    );
    ...
    if (construct?.type !== ABILITY_CONSTRUCT_TYPES.temporaryEffect) return undefined;
    construct.changes.push(createAbilityChange());
    return this.#submitCurrentForm({ "system.constructs": constructs });
  }

  #onDeleteAbilityConstructChange(event) {
    event.preventDefault();
    event.stopPropagation();
    const constructRow = event.currentTarget?.closest?.("[data-ability-construct-row]");
    const constructIndex = Number(constructRow?.dataset.constructIndex ?? -1);
    const changeIndex = Number(
      event.currentTarget?.closest?.("[data-ability-construct-change-row]")?.dataset.changeIndex ?? -1
    );
    const constructs = this.#getSubmittedAbilityConstructs();
    if (constructIndex < 0 || changeIndex < 0 || !constructs[constructIndex]) return undefined;
    constructs[constructIndex].changes?.splice(changeIndex, 1);
    return this.#submitCurrentForm({ "system.constructs": constructs });
  }

  #onAddAbilityConstructResource(event) {
    event.preventDefault();
    const constructIndex = Number(
      event.currentTarget?.closest?.("[data-ability-construct-row]")?.dataset.constructIndex ?? -1
    );
    const constructs = this.#getSubmittedAbilityConstructs();
    const construct = constructs[constructIndex];
    if (construct?.type !== ABILITY_CONSTRUCT_TYPES.resourceChange) return undefined;
    construct.resources.push(createAbilityConstructResource());
    return this.#submitCurrentForm({ "system.constructs": constructs });
  }

  #onDeleteAbilityConstructResource(event) {
    event.preventDefault();
    event.stopPropagation();
    const constructRow = event.currentTarget?.closest?.("[data-ability-construct-row]");
    const constructIndex = Number(constructRow?.dataset.constructIndex ?? -1);
    const resourceIndex = Number(
      event.currentTarget?.closest?.("[data-ability-construct-resource-row]")?.dataset.resourceIndex ?? -1
    );
    const constructs = this.#getSubmittedAbilityConstructs();
    if (constructIndex < 0 || resourceIndex < 0 || !constructs[constructIndex]) return undefined;
    constructs[constructIndex].resources?.splice(resourceIndex, 1);
    return this.#submitCurrentForm({ "system.constructs": constructs });
  }

  #onAddAbilityTriggerCost(event) {
    event.preventDefault();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    if (condition?.type !== ABILITY_CONDITION_TYPES.triggerCost) return undefined;
  ...
  #onDeleteAbilityTriggerCost(event) {
    event.preventDefault();
    event.stopPropagation();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    const costIndex = Number(event.currentTarget?.closest?.("[data-ability-trigger-cost-row]")?.dataset.costIndex ?? -1);
    if (condition?.type !== ABILITY_CONDITION_TYPES.triggerCost || costIndex < 0) return undefined;
    const costs = normalizeAbilityTriggerCostRows(condition.costs);
  ...
  #onAddAbilityEventTrackingTarget(event) {
    ...
    const values = normalizeAbilityConditionValues(condition.trackingTargets)
      .filter(group => ABILITY_EVENT_TRACKING_TARGETS.includes(group));
    const next = getFirstUnusedAbilityEventTrackingTarget(values);
    if (!next) return undefined;
    condition.trackingTargets = [...values, next];
    return this.#submitCurrentForm({ [functionPath]: functions });
  }

  #onDeleteAbilityEventTrackingTarget(event) {
    event.preventDefault();
    event.stopPropagation();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    const index = Number(event.currentTarget?.closest?.("[data-event-tracking-target-index]")?.dataset.eventTrackingTargetIndex ?? -1);
  ...
  #onAddAbilityEventSkill(event) {
    ...
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    if (!condition || condition.type !== ABILITY_CONDITION_TYPES.eventReaction) return undefined;
    if (!getEventReactionDepthProfile(condition.eventKey).skillKeys) return undefined;
  ...
  #onAddAbilityEventExpectedResult(event) {
    event.preventDefault();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    if (!condition || condition.type !== ABILITY_CONDITION_TYPES.eventReaction) return undefined;
    if (!getEventReactionDepthProfile(condition.eventKey).expectedResultKeys) return undefined;
    ...
    if (!next) return undefined;
    condition.expectedResultKeys = [...values, next];
    return this.#submitCurrentForm({ [functionPath]: functions });
  }

  #onDeleteAbilityEventExpectedResult(event) {
    event.preventDefault();
    event.stopPropagation();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    const index = Number(event.currentTarget?.closest?.("[data-event-expected-result-index]")?.dataset.eventExpectedResultIndex ?? -1);
    if (!condition || condition.type !== ABILITY_CONDITION_TYPES.eventReaction || index < 0) return undefined;
    const values = normalizeEventReactionExpectedResultKeys(condition.expectedResultKeys);
    values.splice(index, 1);
  ...
  #onAddAbilityEventDepthFilter(event) {
    event.preventDefault();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    if (!condition || condition.type !== ABILITY_CONDITION_TYPES.eventReaction) return undefined;
    const storageKey = String(event.currentTarget?.dataset.eventDepthStorageKey ?? "").trim();
  ...
  #onDeleteAbilityEventDepthFilter(event) {
    event.preventDefault();
    event.stopPropagation();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    const row = event.currentTarget?.closest?.("[data-event-depth-filter-index]");
    const index = Number(row?.dataset.eventDepthFilterIndex ?? -1);
    const storageKey = String(row?.dataset.eventDepthStorageKey ?? "").trim();
    if (!condition || condition.type !== ABILITY_CONDITION_TYPES.eventReaction || !storageKey || index < 0) return undefined;
    const values = getEventReactionDepthFilterValues(condition, storageKey);
    values.splice(index, 1);
  ...
  #onAddAbilityRegionDamageType(event) {
    event.preventDefault();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    if (!condition) return undefined;
    const values = normalizeAbilityConditionValues(condition.damageTypeKeys);
    const next = getFirstUnusedAbilityRegionDamageTypeKey(values);
    if (!next) return undefined;
    condition.damageTypeKeys = [...values, next];
    return this.#submitCurrentForm({ [functionPath]: functions });
  }

  #onDeleteAbilityRegionDamageType(event) {
    event.preventDefault();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    const index = Number(event.currentTarget?.closest?.("[data-region-damage-type-index]")?.dataset.regionDamageTypeIndex ?? -1);
    if (!condition || index < 0) return undefined;
    const values = normalizeAbilityConditionValues(condition.damageTypeKeys);
    values.splice(index, 1);
  ...
  #onAddAbilityRegionSpecialProperty(event) {
    event.preventDefault();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    if (!condition) return undefined;
    const values = normalizeAbilityConditionValues(condition.regionSpecialPropertyTypes);
    const next = getFirstUnusedAbilityRegionSpecialPropertyType(values);
    if (!next) return undefined;
    condition.regionSpecialPropertyTypes = [...values, next];
    return this.#submitCurrentForm({ [functionPath]: functions });
  }

  #onDeleteAbilityRegionSpecialProperty(event) {
    event.preventDefault();
    const { condition, functions, functionPath } = this.#getAbilityConditionForEvent(event);
    const index = Number(event.currentTarget?.closest?.("[data-region-special-property-index]")?.dataset.regionSpecialPropertyIndex ?? -1);
    if (!condition || index < 0) return undefined;
    const values = normalizeAbilityConditionValues(condition.regionSpecialPropertyTypes);
    values.splice(index, 1);
  ...
  async #onAbilityAuraModeChange(event) {
    ...
    if (event.currentTarget.value !== ABILITY_AURA_MODES.applyToTargets) {
  ...
  #getSubmittedAbilityConstructs() {
    if (!this.form) return normalizeAbilityConstructs(this.item.system?.constructs);
    const formData = new FormDataExtended(this.form);
    const submitData = this._processFormData(null, this.form, formData);
    return normalizeAbilityConstructs(
  ...
  #getSubmittedWeaponFunctionData(path = "") {
    const current = foundry.utils.getProperty(this.item, path) ?? {};
    if (!this.form) return current;
    const formData = new FormDataExtended(this.form);
    const submitData = this._processFormData(null, this.form, formData);
    return foundry.utils.getProperty(submitData, path) ?? current;
  }
  ...
  async #onAddWeaponAdditionalProficiency(event) {
    event.preventDefault();
    const propertyIndex = Number(event.currentTarget?.dataset?.addWeaponAdditionalProficiency);
    if (!Number.isInteger(propertyIndex) || propertyIndex < 0) return undefined;
    const section = getWeaponFunctionSection(event.currentTarget);
    const path = getWeaponFunctionPath(section);
    if (isAttackActionSettingsSection(section)) {
      const settings = this.#getSubmittedAttackActionSettings(path);
      ...
      const next = getFirstUnusedWeaponAdditionalProficiencyKey(settings.proficiencyKey, keys);
      if (!property || !next) return undefined;
      property.proficiencyKeys = [...keys, next];
      await this.#submitCurrentForm({ [path]: settings });
      return this.render();
    }
    await this.#submitQueue;
    const source = this.#getSubmittedWeaponFunctionData(path);
    const properties = normalizeWeaponSpecialProperties(source.specialProperties);
    const property = properties[propertyIndex];
    const keys = normalizeWeaponAdditionalProficiencyKeys(property?.proficiencyKeys);
    const next = getFirstUnusedWeaponAdditionalProficiencyKey(source.proficiencyKey, keys);
    if (!property || !next) return undefined;
    property.proficiencyKeys = [...keys, next];
    await this.item.update({ [`${path}.specialProperties`]: properties });
    return this.render();
  }

  async #onDeleteWeaponAdditionalProficiency(event) {
    event.preventDefault();
    const propertyIndex = Number(event.currentTarget?.dataset?.propertyIndex);
    const proficiencyIndex = Number(event.currentTarget?.dataset?.deleteWeaponAdditionalProficiency);
    if (!Number.isInteger(propertyIndex) || propertyIndex < 0 || !Number.isInteger(proficiencyIndex) || proficiencyIndex < 0) return undefined;
    const section = getWeaponFunctionSection(event.currentTarget);
    const path = getWeaponFunctionPath(section);
    if (isAttackActionSettingsSection(section)) {
      const settings = this.#getSubmittedAttackActionSettings(path);
...
function prepareAbilityFunctionRowsForDisplay(entry, functionIndex = 0, functionPath = "system.functions", item = null) {
  ...
  const fixedExperimentalSurgerySettings = fixedKey === ABILITY_FIXED_FUNCTION_KEYS.experimentalSurgery
    ? normalizeExperimentalSurgerySettings(entry?.fixedSettings)
    : null;
  const fixedEmergencyOperationsSettings = fixedKey === ABILITY_FIXED_FUNCTION_KEYS.emergencyOperations
    ? prepareEmergencyOperationsSettingsForDisplay(entry?.fixedSettings)
    : null;
  ...
  const fixedPainLordSettings = fixedKey === ABILITY_FIXED_FUNCTION_KEYS.painLord
    ? normalizePainLordSettings(entry?.fixedSettings)
    : null;
  ...
  const hasTriggerCostCondition = (entry?.conditions ?? [])
...
function prepareItemAbilityActionForDisplay(
  action,
  index,
  functionIndex,
  functionPath,
  { allowEventSkillCheck = false, allowTreatmentClassShift = false } = {}
) {
  ...
  if (isEventSkillCheck) {
    return {
      ...
      skillChoices: [
        ...
        ...getSkillSettings().map(skill => ({
          value: skill.key,
          label: skill.label || skill.key,
          selected: skill.key === action?.skillKey
        }))
      ],
  ...
  const choices = [
    ...
    ...buildAbilityWeaponActionEntries()
  ];
  return {
    ...action,
    index,
...
function prepareAbilityTriggerCostsForDisplay(costs = []) {
  return normalizeAbilityTriggerCostRows(costs).map((cost, index) => {
...
function prepareActiveApplicationCostsForDisplay(costs = []) {
...
function normalizeAbilityTriggerCostRows(costs = []) {
  const rows = Array.isArray(costs) ? costs : Object.values(costs ?? {});
  return rows.map(cost => {
    const overloadDurationSeconds = Math.max(0, toInteger(cost?.overloadDurationSeconds));
    return {
      id: String(cost?.id ?? "").trim() || foundry.utils.randomID(),
      resourceKey: String(cost?.resourceKey ?? "").trim(),
      formula: String(cost?.formula ?? "0").trim(),
      overloadAmount: overloadDurationSeconds > 0 ? Math.max(0, toInteger(cost?.overloadAmount)) : 0,
...
function prepareInconspicuousSettingsForDisplay(settings = {}, abilityFunction = {}, item = null) {
  ...
  const stateIsCurrent = isInconspicuousRoundStateCurrent(state, combat);
  const attacked = stateIsCurrent && state.attacked;
  return {
    ...normalized,
...
function prepareAbilityConditionForDisplay(condition, functionIndex, index, {
  ...
  const eventDepthFilterGroups = isEventReaction
    ? buildEventReactionDepthFilterGroups(condition, condition?.eventKey, {
  ...
  return {
    ...
    triggerCosts: isTriggerCost ? prepareAbilityTriggerCostsForDisplay(condition?.costs) : [],
    reactionMode,
    ...
    hiddenEventSkillRows: isEventReaction && !showEventSkillFilters
      ? eventSkillKeys.map((value, rowIndex) => ({ value, index: rowIndex }))
      : [],
    showEventExpectedResultFilters,
    eventExpectedResultRows: showEventExpectedResultFilters
      ? buildAbilityEventReactionExpectedResultRows(eventExpectedResultKeys)
      : [],
    canAddEventExpectedResult: eventDisplay.showEventExpectedResultFilters
      ? Boolean(getFirstUnusedAbilityEventReactionExpectedResultKey(eventExpectedResultKeys))
      : false,
    hiddenEventExpectedResultRows: isEventReaction && !showEventExpectedResultFilters
      ? eventExpectedResultKeys.map((value, rowIndex) => ({ value, index: rowIndex }))
      : [],
    ...
    trialEntryRows: buildItemTrialEntryRows(condition?.trialEntries, {
      functionPath,
      functionIndex,
    ...
    regionDamageTypeRows: buildAbilityRegionDamageTypeRows(condition?.damageTypeKeys),
    canAddRegionDamageType: Boolean(getFirstUnusedAbilityRegionDamageTypeKey(condition?.damageTypeKeys)),
    regionSpecialPropertyRows: buildAbilityRegionSpecialPropertyRows(condition?.regionSpecialPropertyTypes),
    canAddRegionSpecialProperty: Boolean(
      getFirstUnusedAbilityRegionSpecialPropertyType(condition?.regionSpecialPropertyTypes)
    ),
    ...
    auraAllowUnconsciousChoices: buildAbilityBooleanChoices(condition?.auraAllowUnconscious === true),
    auraAllowDeadChoices: buildAbilityBooleanChoices(condition?.auraAllowDead === true),
    auraIgnoreHiddenChoices: buildAbilityBooleanChoices(condition?.auraIgnoreHidden !== false),
    auraTriggerOnCreateChoices: buildAbilityBooleanChoices(condition?.auraTriggerOnCreate !== false),
    auraTriggerOnEnterChoices: buildAbilityBooleanChoices(condition?.auraTriggerOnEnter !== false),
...
function buildAbilityEventReactionExpectedResultRows(value = []) {
  const selected = normalizeEventReactionExpectedResultKeys(value);
  return selected.map((resultKey, index) => ({
    index,
    choices: buildAbilityEventReactionExpectedResultChoices(resultKey, selected)
  }));
}

function buildAbilityEventReactionExpectedResultChoices(selectedKey = "", selectedKeys = []) {
  const selected = String(selectedKey ?? "").trim();
  const taken = new Set(normalizeEventReactionExpectedResultKeys(selectedKeys));
  const labels = {
    criticalFailure: localizeAbilityCatalogValue("FALLOUTMAW.SkillCheck.CriticalFailure", "Critical failure"),
    failure: localizeAbilityCatalogValue("FALLOUTMAW.SkillCheck.Failure", "Failure"),
    success: localizeAbilityCatalogValue("FALLOUTMAW.SkillCheck.Success", "Success"),
    criticalSuccess: localizeAbilityCatalogValue("FALLOUTMAW.SkillCheck.CriticalSuccess", "Critical success")
  };
...
function getFirstUnusedAbilityEventReactionExpectedResultKey(value = []) {
...
function buildAbilityEventTrackingTargetRows(value = []) {
  const selected = normalizeAbilityConditionValues(value).filter(group => ABILITY_EVENT_TRACKING_TARGETS.includes(group));
  return selected.map((group, index) => ({
    index,
    choices: ABILITY_EVENT_TRACKING_TARGETS.map(entry => ({
      value: entry,
      label: getAbilityEventTrackingTargetLabel(entry),
...
function getAbilityEventTrackingTargetLabel(group = "") {
  return {
...
function getFirstUnusedAbilityEventTrackingTarget(value = []) {
  const selected = new Set(normalizeAbilityConditionValues(value).filter(group => ABILITY_EVENT_TRACKING_TARGETS.includes(group)));
  return ABILITY_EVENT_TRACKING_TARGETS.find(group => !selected.has(group)) ?? "";
}
...
function prepareItemAbilityConstructForDisplay(construct, index) {
  ...
  return {
    ...
    changes: normalized.changes.map((change, changeIndex) => ({
      ...change,
      index: changeIndex,
      basePath: `${basePath}.changes.${changeIndex}`,
      priority: change?.priority ?? "",
      typeChoices: buildAbilityChangeTypeChoices(change?.type)
    })),
...
function buildItemTrialEntryRows(value = [], {
  functionPath = "system.functions",
  functionIndex = 0,
  conditionIndex = 0
} = {}) {
  const basePath = `${functionPath}.${functionIndex}.conditions.${conditionIndex}.trialEntries`;
...
function syncItemAbilityActionCostVisibility(select) {
...
function syncItemAbilityAttackChoiceControls(select) {
  const actionRow = select?.closest?.("[data-ability-action-row]");
  if (!actionRow) return;
  const selects = Array.from(actionRow.querySelectorAll("[data-ability-attack-choice]"));
  const selected = new Set(selects.map(entry => String(entry.value ?? "")));
  const locked = selected.has(ABILITY_ATTACK_ACTION_ALL);
  const add = actionRow.querySelector("[data-add-ability-attack-choice]");
  if (add) add.disabled = locked || selected.size >= ABILITY_ATTACKING_WEAPON_ACTION_KEYS.length;
  for (const button of actionRow.querySelectorAll("[data-delete-ability-attack-choice]")) {
...
function buildAbilityRegionDamageTypeRows(value = []) {
  const selected = normalizeAbilityConditionValues(value);
  return selected.map((damageTypeKey, index) => ({
    index,
    choices: getAbilityRegionDamageTypesWithSelected(damageTypeKey).map(entry => ({
      value: entry.key,
      label: entry.label || entry.key,
...
function getAbilityRegionDamageTypesWithSelected(selected = "") {
  const entries = [...getConfigurableDamageTypes(getDamageTypeSettings())];
  if (selected && !entries.some(entry => entry.key === selected)) entries.push({ key: selected, label: selected });
  return entries;
}

function getFirstUnusedAbilityRegionDamageTypeKey(value = []) {
  const selected = new Set(normalizeAbilityConditionValues(value));
  return getConfigurableDamageTypes(getDamageTypeSettings())
    .find(entry => !selected.has(entry.key))?.key ?? "";
}

function buildAbilityRegionSpecialPropertyRows(value = []) {
  const selected = normalizeAbilityConditionValues(value);
...
function getFirstUnusedAbilityRegionSpecialPropertyType(value = []) {
  const selected = new Set(normalizeAbilityConditionValues(value));
...
function normalizeSubmittedAbilityItemUseConditions(form = null, submitData = {}) {
  for (const row of form?.querySelectorAll?.("[data-ability-condition-row]") ?? []) {
    ...
    if (type !== ABILITY_CONDITION_TYPES.itemUse && type !== ABILITY_CONDITION_TYPES.duration) continue;
...
function normalizeSubmittedAbilityConstructDurations(form = null, submitData = {}) {
  for (const row of form?.querySelectorAll?.("[data-ability-construct-row]") ?? []) {
    const secondsInput = row.querySelector("[data-ability-construct-duration-seconds]");
    const amountInput = row.querySelector("[data-ability-construct-duration-amount]");
    const path = String(secondsInput?.name ?? "").trim();
    if (!path || !amountInput) continue;
    foundry.utils.setProperty(
      submitData,
      path,
      abilityDurationPartsToSeconds(
        amountInput.value,
        row.querySelector("[data-ability-construct-duration-unit]")?.value
      )
    );
  }
}

function normalizeSubmittedTriggerCostConditions(form = null, submitData = {}) {
  for (const conditionRow of form?.querySelectorAll?.("[data-ability-condition-row]") ?? []) {
    const type = conditionRow.querySelector("input[name$='.type'], select[name$='.type']")?.value;
    if (type !== ABILITY_CONDITION_TYPES.triggerCost) continue;

    const functionRow = conditionRow.closest("[data-ability-function-row]");
    const functionPath = String(functionRow?.dataset.functionPath ?? "");
    const functionIndex = Number(functionRow?.dataset.functionIndex ?? -1);
...
function readAbilityTriggerCostRow(costRow, index = 0) {
  ...
  const payerInput = costRow.querySelector("[data-ability-active-cost-payer]");
  return {
    id: String(
    ...
    overloadAmount: overloadDurationSeconds > 0
      ...
      : 0,
    overloadDurationSeconds,
    ...(payerInput ? { payer: String(payerInput.value ?? "").trim() } : {})
  };
}
...
function normalizeSubmittedEventReactionFunctions(form = null, submitData = {}) {
  for (const row of form?.querySelectorAll?.("[data-ability-function-row][data-function-type='effectChanges']") ?? []) {
    ...
    for (const conditionRow of row.querySelectorAll("[data-ability-condition-row]") ?? []) {
      const type = conditionRow.querySelector("input[name$='.type'], select[name$='.type']")?.value;
...
function normalizeSubmittedAbilityActionCheckboxes(form = null, submitData = {}) {
  const selector = [
    "input[type='checkbox'][name$='.routeAutoRotate']",
  ...
  for (const row of form?.querySelectorAll?.("[data-ability-action-row]") ?? []) {
    const typeInput = row.querySelector("input[name$='.type'], select[name$='.type']");
    if (typeInput?.value !== ABILITY_ACTION_TYPES.treatmentClassShift) continue;
    const typePath = String(typeInput.name ?? "");
    if (!typePath.endsWith(".type")) continue;
    const actionPath = typePath.slice(0, -".type".length);
    const itemTypes = Array.from(
      row.querySelectorAll("[data-ability-treatment-class-item-type]") ?? []
    )
      .filter(input => input.checked)
      .map(input => String(input.value ?? "").trim())
      .filter(type => ["trauma", "disease"].includes(type));
    foundry.utils.setProperty(submitData, `${actionPath}.treatmentClassShift.itemTypes`, itemTypes);
  }
}

function normalizeSubmittedAbilityTrialConditions(form = null, submitData = {}) {
  for (const row of form?.querySelectorAll?.("[data-ability-condition-row]") ?? []) {
    const typeInput = row.querySelector("input[name$='.type'], select[name$='.type']");
    if (typeInput?.value !== ABILITY_CONDITION_TYPES.trial) continue;
    const typePath = String(typeInput.name ?? "");
    if (!typePath.endsWith(".type")) continue;
...
function normalizeSubmittedFixedAbilityFunctions(form = null, submitData = {}) {
  for (const row of form?.querySelectorAll?.("[data-ability-function-row][data-function-type='fixed']") ?? []) {
    ...
    if (fixedKey === ABILITY_FIXED_FUNCTION_KEYS.emergencyOperations) {
      const overloadDurationSeconds = abilityDurationPartsToSeconds(
        row.querySelector("[data-fixed-emergency-operations-overload-duration-amount]")?.value,
        row.querySelector("[data-fixed-emergency-operations-overload-duration-unit]")?.value
      );
      foundry.utils.setProperty(submitData, `${functionPath}.${functionIndex}.fixedSettings.overloadDurationSeconds`, overloadDurationSeconds);
      continue;
    }
    ...
    if (fixedKey === ABILITY_FIXED_FUNCTION_KEYS.inconspicuous) {
      const stealthBonusDurationSeconds = abilityDurationPartsToSeconds(
        row.querySelector("[data-fixed-inconspicuous-duration-amount]")?.value,
        row.querySelector("[data-fixed-inconspicuous-duration-unit]")?.value
      );
      foundry.utils.setProperty(
        submitData,
...
function buildAttackActionSpecialPropertyRows(settings = {}) {
  ...
  return properties.map((property, index) => {
    ...
    return {
      ...
      isLimbDamageMultipliers: type === WEAPON_SPECIAL_PROPERTIES.limbDamageMultipliers,
      limbDamageMultiplierRows: type === WEAPON_SPECIAL_PROPERTIES.limbDamageMultipliers
        ? buildWeaponLimbDamageMultiplierRows(property)
        : [],
      isAdditionalProficiencies: type === WEAPON_SPECIAL_PROPERTIES.additionalProficiencies
        && weaponProficienciesEnabled,
      additionalProficiencyRows: buildWeaponAdditionalProficiencyRows(property, settings.proficiencyKey),
      criticalDamage: {
...
function buildAttackActionSpecialPropertyChoices(selected, properties = []) {
  ...
  return [
    ...
    {
      value: WEAPON_SPECIAL_PROPERTIES.limbDamageMultipliers,
      label: game.i18n.localize("FALLOUTMAW.Item.WeaponSpecialLimbDamageMultipliers")
    },
    {
...
function buildWeaponSpecialPropertyRowsForData(weaponData) {
  ...
  return properties.map((property, index) => {
    ...
    return {
      ...
      isAdditionalProficiencies: type === WEAPON_SPECIAL_PROPERTIES.additionalProficiencies
        && weaponProficienciesEnabled,
      additionalProficiencyRows: buildWeaponAdditionalProficiencyRows(property, weaponData?.proficiencyKey),
...
function buildWeaponSpecialPropertyChoices(selected, properties = []) {
  ...
  const weaponProficienciesEnabled = getActiveRulesProfile().weaponProficienciesEnabled !== false;
    ...
    {
      value: WEAPON_SPECIAL_PROPERTIES.impactConditionWear,
      label: game.i18n.localize("FALLOUTMAW.Item.WeaponSpecialImpactConditionWear")
    },
    {
      value: WEAPON_SPECIAL_PROPERTIES.limbDamageMultipliers,
      label: game.i18n.localize("FALLOUTMAW.Item.WeaponSpecialLimbDamageMultipliers")
...
function buildWeaponLimbDamageMultiplierRows(property = {}) {
  const multipliers = normalizeWeaponLimbDamageMultipliers(property?.limbDamageMultipliers);
  return STANDARD_WEAPON_LIMB_KEYS.map(key => ({
    key,
    label: game.i18n.localize(`FALLOUTMAW.Item.WeaponLimbKeys.${key}`),
    value: Object.hasOwn(multipliers, key) ? multipliers[key] : 1
  }));
}
...
function buildWeaponAdditionalProficiencyRows(property = {}, primaryKey = "") {
  const keys = normalizeWeaponAdditionalProficiencyKeys(property?.proficiencyKeys);
  const settings = getProficiencySettings();
  const used = new Set(keys);
  const primary = String(primaryKey ?? "").trim();
  return keys.map((key, index) => ({
    index,
    choices: settings.map(proficiency => ({
...
function getFirstUnusedWeaponAdditionalProficiencyKey(primaryKey = "", keys = []) {
  const used = new Set([String(primaryKey ?? "").trim(), ...normalizeWeaponAdditionalProficiencyKeys(keys)]);
  return getProficiencySettings().find(proficiency => !used.has(proficiency.key))?.key ?? "";
}
...
function buildItemCategoryChoices(selectedCategory = "", itemCategories = getItemCategorySettings()) {
  const selected = String(selectedCategory ?? "").trim();
...
function buildItemSubcategoryChoices(selectedCategory = "", selectedSubcategory = "", itemCategories = getItemCategorySettings()) {
  const categoryLabel = String(selectedCategory ?? "").trim();
  const selected = String(selectedSubcategory ?? "").trim();
  const category = itemCategories.find(entry => String(entry?.label ?? "").trim() === categoryLabel);
  const subcategories = (category?.subcategories ?? [])
    .map(entry => String(entry?.label ?? entry ?? "").trim())
    .filter(Boolean);

  return [
    { value: "", label: "", selected: !subcategories.includes(selected) },
...
function normalizeSubmittedItemSubcategory(submitData = {}, item = null) {
  const hasCategory = foundry.utils.hasProperty(submitData, "system.itemCategory");
  const hasSubcategory = foundry.utils.hasProperty(submitData, "system.itemSubcategory");
  if (!hasCategory && !hasSubcategory) return;
  const categoryLabel = String(hasCategory
    ? foundry.utils.getProperty(submitData, "system.itemCategory")
    : item?.system?.itemCategory ?? "").trim();
  const subcategory = String(hasSubcategory
    ? foundry.utils.getProperty(submitData, "system.itemSubcategory")
    : item?.system?.itemSubcategory ?? "").trim();
  const category = getItemCategorySettings().find(entry => String(entry?.label ?? "").trim() === categoryLabel);
  const allowed = new Set((category?.subcategories ?? []).map(entry => String(entry?.label ?? entry ?? "").trim()));
  foundry.utils.setProperty(submitData, "system.itemSubcategory", allowed.has(subcategory) ? subcategory : "");
}
