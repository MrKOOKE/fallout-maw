...
export class CombatantPortrait {
    ...
    async renderInner() {
        ...
        const template = await foundry.applications.handlebars.renderTemplate(`systems/${MODULE_ID}/templates/apps/combat-carousel/combatant-portrait.hbs`, { ...data });
        const tooltip = await foundry.applications.handlebars.renderTemplate(`systems/${MODULE_ID}/templates/apps/combat-carousel/combatant-tooltip.hbs`, { ...data });
    ...
    getResource(resource = null, primary = false) {
        ...
        resource = resource ?? this.combat.settings?.resource ?? "resources.health";
