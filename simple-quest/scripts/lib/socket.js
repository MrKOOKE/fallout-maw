import { SOCKET_CHANNEL, SOCKET_SCOPE } from "../constants.js";
...
class SynchronizedStore {
    constructor(storeName, initialValue, callback) {
        ...
        game.socket.emit(SOCKET_CHANNEL, { scope: SOCKET_SCOPE, __$socketOptions: { __$storeName: this._storeName, __$request: true, user: game.user.id } });
    }
    ...
    setData(value) {
        ...
        game.socket.emit(SOCKET_CHANNEL, { scope: SOCKET_SCOPE, __$socketOptions: { __$storeName: this._storeName, user: game.user.id }, data: value });
    }
}
